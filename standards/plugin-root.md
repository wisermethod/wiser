...
This row 1 is the introduction row. This standard declares 1 at introduction; nothing sits behind it. **A contract 2 existed for about an hour on 2026-09-22 and was withdrawn**, along with the C10 amendment it carried. It required a declared `version`; the five roots were converged to it and back. No tree outside this workspace consumed it, and the table returns to one row rather than carrying a number nothing needs.
...
**This clause was reversed on 2026-09-22 and restored the same day, and both moves are recorded so neither is made again from the same evidence.** It was reversed to require a `version`, on the reading that an installed copy in Claude Cowork had frozen at one commit for 113 commits because it declared none. **That reading was wrong.** The `v1` a reader sees in Cowork is the ingest service's own revision counter, not the plugin's declared version: the same record showed `0001` for this root against `0038` for a first-party plugin beside it, and declaring `1.0.0` did not move it. **Cowork does not read this repository at all.** It is served a snapshot that Anthropic's service ingested once, on 2026-09-16, from `/api/organizations/<org>/marketplace/plugin-assets/<id>`, and the repository carries no webhook and no GitHub App that could announce a push. **So the omission never caused the freeze, and declaring a version cannot cure it.**

**What the reversal would have cost is the reason the restoration matters.** A declared version pins the plane that honours it: on a git-sourced install, omitting the key is what lets a host resolve the source commit and follow the default branch, and declaring one freezes every installed copy until someone remembers to raise it. The reversal traded a working mechanism on one host for no gain on another.
