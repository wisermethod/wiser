...
| 2 | `.claude-plugin/plugin.json` declares a `version`, per C10 as amended 2026-09-22, which reversed the clause that forbade it | `skills/Onboard Plugin Root/` at instantiation, `skills/Housekeeping/` afterwards | required |

This row 1 is the introduction row. This standard declares 1 at introduction; nothing sits behind it. **Row 2 is the first row that moves the tree**, and it moves one file by one key: a root at 1 reaches 2 by declaring a `version` in `plugin.json`, and nothing else in the tree changes. The five plugin roots were converged to 2 in the same change that amended C10, so no root was left reading `plugin-old` on a clause it had no way to have satisfied.
...
`plugin.json` carries a `version`, a bare `MAJOR.MINOR.PATCH` with no leading zero on a component and no pre-release or build part. A missing `version`, or one of another shape, scores misfiled. `marketplace.json` carries none; the catalog entry takes its version from the plugin it names, and a `version` on the marketplace file scores misfiled. No Update outcome is asserted here, and none is asserted anywhere in this root.

**This clause said the opposite until 2026-09-22, and the reversal is recorded rather than dropped.** It required both files to omit the key on a git-sourced root and scored its presence misfiled, on the reasoning that the omitted key is what lets a host resolve the source commit. **That reasoning is sound for one host and false for another, which is why the rule could not stay.** Claude Code does resolve the commit: its install record carries a `gitCommitSha` field described as being for version tracking, so a version-less plugin is a shape it supports by design. **Claude Cowork records no commit for a plugin at all.** Its roster carries a server-assigned `updatedAt` and nothing else, it re-downloads only when that value moves, and it presents a version-less plugin to the user as a fixed `v1` with Update greyed out. Measured 2026-09-22: an installed `wiser` sat at `846887d` for 113 commits over six days, pinned by a zero-difference tree comparison, while the catalog reported no change and the operator's Update control did nothing. **A clause may not require a shape that makes a plugin unable to reach the people who installed it on one of the hosts this root ships to.**

**What a version costs, so nobody reverses this without knowing.** A declared number must be raised when a release should reach an installed copy, and a release that forgets it reaches nobody while appearing to succeed. Whoever publishes a root owes that bump, by whatever mechanism they publish through; this clause states the obligation and names no tool, because a root published by some other route owes exactly the same thing.
