 ...
 *
 * The address the caller names is an input from outside, so it is screened
 * here before a browser is launched at it: an address inside the machine or its
 * network is not a web page this tool captures, and the addresses cloud
 * platforms answer instance credentials on are the reason that matters. Copied
 * from sitemap-fetch, which applies the same screen with the same wording.
 *
