...
**Version:** 0.1.6, 2026-09-11. Changelog: Pro has a specified local graph contract; executable graph verbs remain absent.
...
| Pro | `graph`, LadybugDB `graph.lbdb` | Ingest as Candidates under `experts/Memory Expert/graph.md`. Not yet executable; stop before sources | Cypher MATCH plus embedding neighbours, returning located items. Same stop | Human promote; no auto-Canonical |
...
**Graph.** The specified local graph contract is `experts/Memory Expert/graph.md`. Load it for LadybugDB storage, Cypher relation traversal, embedding neighbours, item shapes, Candidate ingest, upgrade seed, and failure semantics. Query, embed, and ingest are English verbs there, not new CLI ids; until the tool ships them, graph paths stop on that contract before sources. The specification is not a claim that graph execution is available.
...
- Graph: the shipped themes may seed Candidate Ideas with located quotes; follow `experts/Memory Expert/graph.md`, including its current execution stop before sources
...
**Databased to graph.** Follow the Candidate seed in `experts/Memory Expert/graph.md`: reuse corpus paths without copying corpus; confirmed canon and decided items seed Candidates, with no silent Canonical copy. Until the tool ships graph ingest, stop before any recipe change or source read and keep the existing backend and compiled layer intact. This contract does not claim an upgrade ran.

**Graph to hosted.** Stop on `experts/Memory Expert/hosted.md` until the hosted connector is specified. No export runs.

Down is not a path. Wiki to graph does not skip databased: go through databased, or follow `experts/Memory Expert/graph.md` and its current execution stop when Pro was requested without a databased set. Knowledge Curation owns upgrade; it is not another skill.

Wiki and databased ship in Memory v1. Graph is specified in `experts/Memory Expert/graph.md` and awaits tool support; hosted remains a stub. The options are not rankings of knowledge quality, and Standard is not gated on a wiki eval failure.

## Honest stops

- `graph`: specified in `experts/Memory Expert/graph.md`; the tool still stops before sources until query, embed, and ingest exist. Load that contract and report its current named execution stop. Once those verbs ship, absent dependencies stop as `missing-engine: ladybug` or `missing-weights` under that contract
