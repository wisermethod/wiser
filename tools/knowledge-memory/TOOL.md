...
| `recall --set DIR --store FILE --query TEXT` | Databased lexical or graph MATCH/embedding items, never an answer; optional `--top-k` (15, and 25 for graph's passage candidate pool), `--as-of YYYY-MM-DD` (recorded, not filtered), and on graph only `--rank MODE` (default `cosine`; `classifier` asks which of the hybrid pool's passages to read in full), `--select FILE-OR-JSON`, `--candidates-only`, and, only with `--rank classifier`, optional `--owning-root DIR` (has to equal the session's owning root; `hooks/AGENTS.md`), `--gateway-home DIR` and `--classifier-record FILE`. The set directory and the store directory are the material, and each has to sit inside that owning root |
...
Graph recall treats `--query` beginning with MATCH, case-insensitively, as Cypher even with `retrieval: embedding`. It opens that file read-only, refuses writes, comments, separators and NUL, and prepares before executing. Return a node, or `name`, `quote`, `source_path` in that order; other result shapes fail. Otherwise `retrieval: embedding` computes local CPU embeddings of `Passage.text`, each passage embedded on its own text alone, with attention-mask mean pooling and L2 normalization, and ranks candidates by the `--rank` mode, default `cosine`; `hybrid` interleaves BM25 with cosine and is computed in process from the same column, loading no weight recall did not already load. `--rank classifier` builds that same hybrid pool, at the same `--top-k`, and records the judgment in Classifier judgment; it does not replace the pool. A store holding no passages falls back to embedding names, definitions or summaries, quotes and aliases, which is what a store ingested before passages contains. `--select` names passages the caller already ranked and returns those in the order named, doing no embedding of its own. **It takes the JSON list itself or a path to a file holding it**: a value whose first non-blank character is `[` is the list, and any other value is a path, screened as every caller-named path is. The inline form exists so a caller need not write which passage answers which question into the tree it is working in. Both forms take the same shape and the same validation, and an empty list is accepted and returns no items. **`--candidates-only` returns the `part: passage` items alone**, unchanged in every field and still ranked 1 to N in order, for a caller that is told to ignore the other two parts; the result then carries `candidates_only`, and the default path is unchanged. Absent `--top-k`, the passage candidate pool takes 25 and the MATCH path and the no-passage fallback take 15; an explicit `--top-k` governs every path, and neither flag changes any of them. `retrieval: lexical` remains the default and is not a graph text-search fallback. Databased rejects `retrieval: embedding`.

Graph items contain `name`, `quote`, `source_path`; embedding items also contain `score` and `rank`. An item from a store holding passages carries `part`, one of `passage`, `idea`, `related` or `match`, and a `related` item carries `relation`, `direction` and `via`; an `idea` or `related` item has no `score` or `rank`. The fallback over a store with no passages returns the flat shape and no `part`. A result also carries `parts`, a count per label and empty on that fallback, plus `selected` on a `--select` call, `ranking` on a non-default `--rank`, `candidates_only` on a `--candidates-only` call, whose `parts` then holds `passage` alone, and `classifier` on `--rank classifier`, so a default-path result is unchanged and any other says on its face what it did. No `answer` field is returned. `--as-of` is recorded, never filtered. Node confirmation is not inferred from a similarity score. Model files must already exist under the person-scoped `models/` folder named by the constitution's Writes and `tools/AGENTS.md`; optional recipe `embedding_file` is a basename there, with no separators or `..`. The tokenizer is `tokenizer.json` beside it. No weights are downloaded, including with `--install`.

## Classifier judgment

`--rank classifier` puts one closed judgment to the classifier: which of the pool's passages to read in full. The pool is the one `--rank hybrid` returns at the same `--top-k`, and `items` is that pool with every score and rank left as the ranker wrote them. The path that settles the judgment without a classifier is that pool, chosen by the caller as `skills/Knowledge Recall/` Step 2's chooser does today. The result records the settlement on `classifier`: `path` (`classifier`, `replay`, or `builtin`), `reason`, `kept` (the passage names kept, in the answer's order, at most three, or `[]`), and `record` (the answer as received, with the question and the candidates, or null when the path is builtin), and `ms`, the judgment's whole wall-clock time. The call adds one candidate after the passages, id `none`, text `None of these passages answers the question.`. If the answer ranks `none` first, nothing is kept: `classifier` says `abstained: true`, `kept` is empty, and no passage is recalled. Knowledge Recall then answers that the store holds no answer to the question, as its own not-found path does. Otherwise the first three passage ids in the answer's order are kept, and `none` is skipped wherever it falls. The replay record carries the candidates, including `none`. A builtin path leaves the choice to that chooser. The owning root comes from the session binding (`hooks/AGENTS.md`). `--owning-root` is optional and has to be that root. The set directory and the store directory are passed as the material, and each has to sit inside that root and not be refused where it sits. The rule is `standards/primitives.md` Invocation.
