...
| `recall --set DIR --store FILE --query TEXT` | Databased lexical or graph MATCH/embedding items, never an answer; optional `--top-k` (15), `--as-of YYYY-MM-DD` (recorded, not filtered), and on graph only `--rank MODE` (default `cosine`) and `--select FILE` |
...
All flags above are closed sets. Every command accepts the bare `--install` flag; only graph ingest, graph recall and `check --install` use it. `--rank` and `--select` are graph-only and are refused by name on a databased recipe, and each is refused on a call it ranks or selects nothing for: `--rank` with MATCH or beside `--select`, `--select` with MATCH. `bootstrap`, `review-pass`, `promote`, `mark-stale`, `healthcheck` and `forget` remain databased-only. Graph recipes on them fail before sources or store creation.
...
On `retrieval: embedding` ingest also cuts each source into `Passage` nodes at the embedder's window, 254 content tokens with 4 tokens of overlap, and writes a `LOCATED_IN` edge from each Idea and Entity to every passage its located quote meets. The covering set is computed twice by independent means and a disagreement fails the ingest, because a wrong window search returns a believable number. Passage numbering continues from the store, so a second source adds rather than collides. A Cypher-only recipe cuts none and needs no weights at ingest. Counts are reported as `passages_inserted`, `located_in_edges` and `passage_spans_unlocatable`.
