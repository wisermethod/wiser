...
`--rank classifier` puts one closed judgment to the classifier: which of the pool's passages to read in full. The pool is the one `--rank hybrid` returns at the same `--top-k`, and `items` is that pool with every score and rank left as the ranker wrote them. The path that settles the judgment without a classifier is that pool, chosen by the caller as `skills/Knowledge Recall/` Step 2's chooser does today. The result records the settlement on `classifier`: `path` (`classifier`, `replay`, or `builtin`), `reason`, `kept` (the passage names kept, in the answer's order, at most three, or `[]`), and `record` (the answer as received, with the question and the candidates, or null when the path is builtin), and `ms`, the judgment's whole wall-clock time. A builtin path leaves the choice to that chooser. Nothing is sent without an owning root that does not refuse. The rule is `standards/primitives.md` Invocation.
