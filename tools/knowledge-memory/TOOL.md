...
| `recall --set DIR --store FILE --query TEXT` | Databased lexical or graph MATCH/embedding items, never an answer; optional `--top-k` (15, and 25 for graph's passage candidate pool), `--as-of YYYY-MM-DD` (recorded, not filtered), and on graph only `--rank MODE` (default `cosine`; `classifier` asks which of the hybrid pool's passages to read in full), `--select FILE-OR-JSON`, `--candidates-only`, and, only with `--rank classifier`, optional `--owning-root DIR` (checked against the session), `--gateway-home DIR` and `--classifier-record FILE`. The set directory and the store directory are the material that judgment is about |
...
`--rank classifier` puts one closed judgment to the classifier: which of the pool's passages to read in full. The pool is the one `--rank hybrid` returns at the same `--top-k`, and `items` is that pool with every score and rank left as the ranker wrote them. The path that settles the judgment without a classifier is that pool, chosen by the caller as `skills/Knowledge Recall/` Step 2's chooser does today. The result records the settlement on `classifier`: `path` (`classifier`, `replay`, or `builtin`), `reason`, `kept` (the passage names kept, in the answer's order, at most three, or `[]`), and `record` (the answer as received, with the question and the candidates, or null when the path is builtin), and `ms`, the judgment's whole wall-clock time. A builtin path leaves the choice to that chooser. The owning root comes from the session binding. `--owning-root` is optional and is checked against that session. The set directory and the store directory are passed as the material, and each has to sit inside one of the session's roots. Nothing is sent when the session does not verify, the root refuses, or the material is outside those roots. The rule is `standards/primitives.md` Invocation.
