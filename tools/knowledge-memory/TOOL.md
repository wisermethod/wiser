...
All flags above are closed sets. Every command accepts the bare `--install` flag; only graph ingest, graph recall and `check --install` use it. `--rank` and `--select` are graph-only and are refused by name on a databased recipe, and each is refused on a call it ranks or selects nothing for: `--rank` with MATCH, beside `--select`, or against a store holding no passages, and `--select` with MATCH. `--rank` is refused on those calls whatever its value, including `cosine`, because a flag accepted and ignored looks like a flag that applied. `bootstrap`, `review-pass`, `promote`, `mark-stale`, `healthcheck` and `forget` remain databased-only. Graph recipes on them fail before sources or store creation.
