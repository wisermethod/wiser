...
| `recall --set DIR --store FILE --query TEXT` | Databased lexical or graph MATCH/embedding items, never an answer; optional `--top-k` (15, and 25 for graph's passage candidate pool), `--as-of YYYY-MM-DD` (recorded, not filtered), and on graph only `--rank MODE` (default `cosine`) and `--select FILE` |
