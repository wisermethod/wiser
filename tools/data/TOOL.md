...
An accepted answer for a column is an entry for that column's id whose `choice` is `quantity`, `identifier`, `year`, `code`, or `flag`. `none`, a missing entry, an entry status, and any other choice leave that column's role null. No verified session, no single owning root, a refusal, no classifier, or a judgment that accepts nothing: `path` is `builtin` and every role is null. The data file is the material the judgment is about, and it has to sit inside the session's owning root. `--owning-root` is optional and has to be that root. The binding is `hooks/AGENTS.md`. The rule is `standards/primitives.md` Invocation.
...
| `--owning-root <dir>` | Optional absolute path. It has to equal the session's owning root. The data file is the material. A mismatch, an unreadable root, or a refusal makes no call | None |
