...
// Playwright lives in the shared runtime; mermaid lives in this tool. An
// interrupted install leaves node_modules/ behind with nothing in it, so the
// directory proves nothing, and each package is checked in the directory that
// holds it.
const DEP_MARKERS = {
  playwright: join(RUNTIME_DIR, 'node_modules', 'playwright', 'package.json'),
  mermaid: join(TOOL_DIR, 'node_modules', 'mermaid', 'package.json')
};
...
function npmCi(dir, marker) {
  ...
  if (!existsSync(marker)) {
    fail(`Error: npm ci finished but ${marker} is still missing. Check that package.json lists every package this script imports.`);
  }
}

// Dependencies. Runs before any package import; keep it at the top of every entry script.
const needPlaywright = !existsSync(DEP_MARKERS.playwright);
const needMermaid = !existsSync(DEP_MARKERS.mermaid);
if (needPlaywright || needMermaid) {
  requireInstallConsent('packages');
  if (needPlaywright) npmCi(RUNTIME_DIR, DEP_MARKERS.playwright);
  if (needMermaid) npmCi(TOOL_DIR, DEP_MARKERS.mermaid);
}
