...
function flag(name) {
  ...
  if (argv.indexOf(name, index + 1) !== -1) {
    fail(`Error: ${name} was given more than once and takes one value. Run "node scripts/capture.js help" for usage.`);
  }
...
// Every command of this tool, `check` included, reaches ensureChromium() once
// the install is authorised, so every one of them would fetch the browser.
const WILL_FETCH_BROWSER = true;
...
if (browserSurvey === null && command === 'check' && !existsSync(DEP_MARKER)) {
  browserSurvey = {
    ...
    failure: 'artifact',
    remediation: 'dependency: playwright in the shared browser runtime; check: node_modules/playwright/package.json inside tools/lib/browser-runtime/. Next: run node scripts/capture.js check --install to install the packages and the Chromium build in one authorised run, then report on them.'
  };
}
