...
Every script in this tool follows `system/templates/Script Contract.md`: self-contained imports, help answered before the dependency check, the first-run dependency install, closed unknown flags, and the stdout and stderr rules. It reads one caller-named file and writes one caller-named HTML file outside this tool directory, plus the first-run install into its own directory. The tool carries no Dependencies section because `csv-parse` installs by the first-run check and Node covers the rest. The sections above state what the command does; the contract states how the script behaves getting there.
