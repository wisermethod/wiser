...
Every script in this tool follows `system/templates/Script Contract.md`: self-contained imports, help answered before the dependency check, the consent-gated dependency install, and the stdout and stderr rules. It reads two caller-named files and writes nothing of its own; the writes are what `tools/AGENTS.md` lists, which is the only place this repository states them, so the contract's `--env` and output-path clauses have nothing to bind here, and the tool carries no Dependencies section because `csv-parse` installs by the first-run check and Node covers the rest. The sections above state what the command does; the contract states how the script behaves getting there.
