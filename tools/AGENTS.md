...
| A Chromium build, a few hundred megabytes, **downloaded from `https://cdn.playwright.dev`** (Playwright's own fallback is `playwright.download.prss.microsoft.com`). `playwright install chromium` fetches **three artifacts in sequence** -- Chrome for Testing, FFmpeg and Chrome Headless Shell -- and a default headless launch uses the headless shell, so a download that stops after the first leaves the browser incomplete rather than absent; `playwright install chromium --dry-run` names every artifact and its directory, which is what the tools ask rather than guessing. **Three states, because two of them cost this build a gate round each.** An artifact is complete when Playwright's own `INSTALLATION_COMPLETE` marker is inside its directory -- written last, after the extract, and the only thing `install chromium` looks at before skipping an artifact. *Absent* is no marker: `--install` fetches it. *Incomplete* is a directory with no marker, which is what an interrupted download leaves: `--install` fetches it too, because Playwright removes and recreates the directory before extracting. *Marked but emptied* is a marker whose files are gone -- an antivirus sweep, a partial restore, a hand-cleared cache -- and it is the one that needs `--force`, because Playwright skips a marked artifact and would otherwise do nothing; the tools detect it by asking for the executable as well as the marker, and pass `--force` themselves on the run that authorised the install | wherever Playwright puts it, which this repository does not choose and does not second-guess: `PLAYWRIGHT_BROWSERS_PATH` if that is set to a path, `<tool>/node_modules/playwright-core/.local-browsers` if it is set to `0`, and otherwise the platform default under Playwright's own cache root — `~/Library/Caches/ms-playwright` on macOS, `$XDG_CACHE_HOME/ms-playwright` or `~/.cache/ms-playwright` on Linux, `%LOCALAPPDATA%\ms-playwright` on Windows | `Browser Control`, `deck-export`, `html-to-png`, `mermaid-to-png`, `svg-to-png`, `web-screenshot` |
