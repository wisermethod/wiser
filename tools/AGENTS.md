...
| npm's own download cache and logs | npm's configured cache, `~/.npm` by default, outside this plugin. **The packages come from `https://registry.npmjs.org`**, which is the one host a first run contacts for a non-browser tool | the same 17, on first run |
...
**Those two hosts are the whole of what an install reaches, and they are named here for the same reason the paths are.** A tool that installs contacts `registry.npmjs.org`, and a browser tool also contacts `cdn.playwright.dev`. No tool's own file may say it makes no network request without saying "after the first-run install", because the first run is exactly when it does. An egress allowlist needs those two hosts and nothing else.
