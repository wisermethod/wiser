...
**One host is reached at RUN time rather than install time, and it belongs here for the same reason the install hosts do.** A deck is an HTML file the caller owns, and a deck may name any asset host it likes; the starter deck this repository ships in `skills/Create Presentation/starter-deck.html` names **`cdn.jsdelivr.net`**, which is where it loads reveal.js and its themes from. So `deck-export` and `html-to-png` rendering that deck reach `cdn.jsdelivr.net` on a run that installs nothing, and an egress allowlist built from the install list alone renders a blank deck rather than failing loudly. It is not an install host, no script here fetches from it, and it is the deck's choice rather than this plugin's -- which is exactly why the register has to name it once instead of leaving it to be discovered.
