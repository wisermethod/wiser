...
| A session token, so an unauthenticated caller cannot drive a signed-in browser | `~/.wiser/browser-control/<port>.token`, mode 0600 in a 0700 directory, outside this plugin. Written when a session starts and removed when it stops | `Browser Control` |
