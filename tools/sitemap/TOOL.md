...
Offline use: where is the sitemap? It already sits on disk, as a saved historical export, a body saved because the host refused non-browser clients, or a gzip archive from an earlier pull: pass `--file` and do not open a network connection. The caller wants a live read and has no such file: pass `--domain` or `--url`. Both a local file and a live refresh were asked for: the live refresh wins on that run. Do not pass `--file` with it. You have a file and cannot tell whether it is the sitemap: ask. Do not fetch it to decide. The snapshot shape is the same either way, so `diff` compares an offline historical snapshot to a live one without caring which mode produced each side.
...
Offline mode takes only `--file` seeds: no robots discovery, no http(s) request, and no following of child locs a local index declares. Pass each child sitemap as its own `--file`. What date is stamped? A `--file` seed whose request or file name states the date it represents: pass that `--date`, so a later `diff` compares the right pair of points in time. A fetch just made, or a file the request does not call historical: omit `--date`. The tool stamps today. A file the request calls historical, with no date stated: ask. Do not stamp today on that file.
...
| `count` is far below the site's real page count | An index was deeper than five levels, or `--max` stopped the walk | Is `truncated` true? Yes: raise `--max` and re-run. No, and a child sitemap sits deeper than five levels, which you can see in the index you read: pass those children with `--url` or `--file`. You do not know the site's real page count: deliver this snapshot. Do not raise `--max` on a guess |
...
| A routine re-fetch reports removals in the thousands | The current snapshot is partial: a cap stopped the walk, or a sitemap failed to read, so URLs that still exist were never collected | Does the result carry `sourceIncomplete`? Yes: those removals are a gap in the fetch, not pages the site retired. Re-fetch with a higher `--max`, or once the failing sitemap reads, then diff again. No: report the removals as the diff computed them. The field was not read: read it before you describe the removals. Do not call them retirements while it is unread |
