...
| The GIF is 480 pixels wide when the video was larger | Expected: GIF output is scaled to 480 wide, height following the aspect ratio | Does the request need a width other than 480? `gif` always scales to 480 wide, so resizing first does not change it, and this tool has no other GIF width: say so, and deliver 480 or nothing. No: run `gif` |
