...
| The GIF is 480 pixels wide when the video was larger | Expected: GIF output is scaled to 480 wide, height following the aspect ratio | Does the request name a width other than 480? Yes: `edit` to that width first, then `gif`. No: run `gif`. The width is 480, height following the aspect ratio. The request says the GIF should not be 480 and names no width: ask. Do not pick a width |
