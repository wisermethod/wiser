...
The order, and the reason each failure carries:
...
3. The binding for that session does not verify: the verifier's reason (`no-harness`, `no-pointer`, `stale-session`, `no-binding`, `unverifiable`).
4. The binding is refused: `refused`.
5. The binding's `roots` is empty: `no-owning-root`.
6. `owningRoot`, when the caller names one, has to be one of `roots` after resolving symlinks: otherwise `root-mismatch`. When the caller names none, `owning_root` has to be that one root: otherwise `no-owning-root`.
7. That root's `AGENTS.md` cannot be read: `unreadable-root`. Read again now, a refusal at or above it: `refused`.
8. `material` is an optional list of absolute paths the judgment is about, a file or a directory. Each has to resolve inside one of `roots`: otherwise `material-outside-root`. A refusal at or above that path: `refused`.
9. Then the presence check and the call, as before. No attached classifier: `no-classifier`. A call that returns nothing: `unavailable`. An answer that carries a status: that status.
...
`--material` may be repeated. `--input -` reads stdin. `help` and `--help` print usage and exit 0. An unknown flag, a missing `--action` or `--input`, unparseable input, and a replay mismatch exit 1 with a message on stderr. Every path above exits 0.
