...
`blind` writes one packet per valid run: the ask, the rubric items, and the deliverable, with every sentence that names how a judgment was settled (the classifier, a builtin path, a gateway status, the route line) removed from both arms alike, and the count per packet kept in the map, not the packet. It refuses to write any packet when a deliverable holds the classifier key. The map from opaque id to run id is a separate file, and `blind` prints only counts, so the session driving the trial never sees which run is which. `score` asks for one fenced JSON block and records `scores`, `reasons`, `score`, `usd` (every attempt charged) and the packet's sha256, or `status: unparsed`; it re-scores a packet whose hash changed and stops before its spend would pass `usd_worst`. `report` refuses unless every planned run is valid and fully scored against the packet now on disk, then writes the five pass lines from `standards/primitives.md` `## Classifier Seam`. The host runs with `--include-hook-events`, so the stream carries the route hook's own reply, and each run records `route_answer` (the file the classifier picked, or `none` when the hook printed nothing) and `route_p`; a run whose stream lacks that reply is invalid, and so is an off-arm run the hook routed. On a routing trial line b is the classifier's own answer: every none case answered `none` in every on-arm run. On a seam trial it is judged from each on-arm deliverable by the case's none item, because each tool shapes the answer in its own output; `basis` says which. Every verdict lists `classifier_picks` per case. Plan and verdict dollar figures count host, judge and classifier. `release` is always `a separate decision; this verdict releases nothing`.
