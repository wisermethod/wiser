...
The script in this tool, and the destination screen beside it that the script imports, follow `system/templates/Script Contract.md`: self-contained imports, help answered before anything else, and the stdout and stderr rules. They import no package and read no configuration file, so that contract's dependency check and `--env` clauses have nothing to bind here, and the tool carries no Dependencies section because Node 18 or newer is all it needs; `package.json` is here to declare that and the module form. The sections above state what the command does; the contract states how the script behaves getting there.
