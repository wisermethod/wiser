...
- An image with `alt=""` counts as an image without alt text, though an empty `alt` is the correct markup for a decorative image. Leave that count, and do not add alt text to clear it. When the attribute is absent, the check stands, and do not invent alt text. When the request names that image as decorative, say so to the requester rather than by changing the JSON. Apply this per image, and do not clear the group's issue while any image lacks the attribute.
...
One page per run. `--page-url` is required because internal and external links cannot be told apart without a host to compare against; a caller analyzing a build artifact or a staging copy passes the address the page will be served at. When you do not know that address, ask, and do not pass `file://` or omit the flag.
...
| Every group reports absences on a page that plainly has content | The HTML holds a shell that a browser fills in, or the file is not HTML | When the file holds the title, headings, and body text the page shows, deliver the report, and do not fetch another copy. When it is a shell a browser fills in, hand in the HTML as a browser renders it, not the served source, and run again. When you cannot tell, ask, and do not treat the absences as a rewrite list |
...
| `imagesWithoutAlt` counts images that are decorative | Empty `alt` counts as missing, per What It Checks | Report the count and do not add alt text. When the attribute is absent, the check stands, and do not invent alt text |
