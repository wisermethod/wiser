#!/usr/bin/env node
/**
 * data-aggregate - group a CSV, JSON, or TSV file's rows and aggregate them
 *
 * Usage:
 *   node scripts/aggregate.js help
 *   node scripts/aggregate.js aggregate --file <path> --group-by <column> --metric <column>:<function>
 *
...
const COMMANDS = new Set(['aggregate']);
// Restated here, and only here, because the core module imports csv-parse: it
// cannot be loaded until the dependency check below has passed, and a metric
// naming an unknown function is a usage mistake that should never reach an
// install. The core rejects the same list again for callers that skip this file.
const FUNCTIONS = new Set(['sum', 'mean', 'median', 'min', 'max', 'count']);

const USAGE = `data-aggregate - group a CSV, JSON, or TSV file's rows and aggregate them
...
if (!COMMANDS.has(command)) {
  fail(`Error: unknown command "${command}". Run "node scripts/aggregate.js help" for usage.`);
}
...
// option returns groups that look finished and are not what was asked for.
for (let index = 1; index < argv.length; index += 1) {
  const option = argv[index];
  if (valuePositions.has(index)) continue;
  if (option.startsWith('-') && !VALUE_FLAGS.has(option) && !BARE_FLAGS.has(option)) {
    fail(`Error: unknown option "${option}". Run "node scripts/aggregate.js help" for usage.`);
  }
}
...
function flagValues(name) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (value === undefined || value.startsWith('--')) {
      fail(`Error: ${name} needs a value. Run "node scripts/aggregate.js help" for usage.`);
    }
    found.push(value);
  }
  return found;
...
function flag(name) {
  ...
  if (found.length > 1) {
    fail(`Error: ${name} was given ${found.length} times and takes one value. Run "node scripts/aggregate.js help" for usage.`);
  }
  return found[0];
}
...
// never triggers an install. csv-parse is not needed to know a flag is missing.
const fileArgument = flag('--file');
if (!fileArgument) {
  fail('Error: --file is required. Pass the absolute path to a CSV, JSON, or TSV file. Run "node scripts/aggregate.js help" for usage.');
}
...
// One stat answers both questions the path can fail on, and it answers them
// here rather than after the install: a directory is a usage mistake like any
// other, and a usage mistake never pays for a dependency install.
let fileStat;
try {
