...
Which project is written? The caller named a template path and that path is a file: pass `--template`. The caller named none: omit `--template` and write the starter. The deck must present with no network, and no template was named: the starter. The caller named a template and also said the deck must present with no network: does the template load anything from the network? No, its assets are all local: use it. Yes, or you cannot tell: ask which of the two to write. Do not pick. A named template path that is not a file: stop. Do not substitute the starter.
...
Size comes from the deck. After the deck initializes, its own reveal.js configuration supplies the slide width and height, and the export is made at that size, so a 4:3 deck exports 4:3 without being told. Does the request name a pixel size? Yes: pass `--width` and `--height`. No: omit them, including when the deck's configuration is a percentage, and the tool uses its own fallback. Do not invent a size; read the reported pair back. The reported `width` and `height` are what the export was made at, which is the pair worth reading back.
...
| `the deck did not initialize within <n> ms` | Not a reveal.js deck, an asset missing beside it, a CDN-loaded deck with no network, or a **proxy-only-egress** host without `HTTPS_PROXY` / `HTTP_PROXY` in the environment | Does the file present when opened in a browser? Yes: raise `--timeout` and re-run. No: do not raise the timeout. A missing asset or a file that is not a reveal.js deck is the cause; fix that. Is this host proxy-only, with neither `HTTPS_PROXY` nor `HTTP_PROXY` set? Yes: export the one this host uses, then re-run. The runtime forwards it into Chromium. No: do not set a proxy. You cannot tell whether it presents, or whether egress is proxied: stop and name the check you could not make. Do not re-run with a higher timeout as a guess |
...
| A slide with a progressive reveal exports in its first state | Fragments advance on a click nobody makes here | Does the request need each fragment state as its own image? Yes: split the slide so each state is its own section, then render. No, one image of the final state: drop the fragments, then render. The request does not say: ask. Do not ship the first state as the whole reveal |
| Fonts look wrong | A webfont the deck names was not reachable | Is the font file one you can place beside the deck? Yes: reference it by a relative path and re-render. No: deliver the fallback and say the webfont was not reachable. You cannot tell: ask. Do not fetch the webfont as a substitute |
