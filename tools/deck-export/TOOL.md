...
Size comes from the deck. After the deck initializes, its own reveal.js configuration supplies the slide width and height, and the export is made at that size, so a 4:3 deck exports 4:3 without being told. Does the request name a pixel size, or does the deck's configuration set a percentage rather than a pixel count? Yes: pass `--width` and `--height`. No: omit them. The deck's configuration is the size. You cannot read the configuration, and the request names no size: omit them. Do not invent a size. The reported `width` and `height` are what the export was made at, which is the pair worth reading back.
