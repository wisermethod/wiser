...
Do not use it to fetch anything. It authenticates to nothing, holds no credential, and makes no network request; the caller pulls the rows and hands them in as files. Do not use it to judge: it computes what the supplied rows contain and returns no findings, no priorities, and no recommendations, so the SEO judgment that reads this dataset lives with its caller. Do not reach for it for one page's on-page elements; that is `seo-page-analyzer`. Do not expect backlink data or competitive figures, which appear in neither of the two sources the audit dataset is built from.
