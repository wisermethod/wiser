...
- The request names a topic, a directory, or a set of files handed in by path, and not also the whole workspace: constrain the analysis to it. It names the whole workspace and nothing narrower: read across the composed roots. Both a narrower scope and the whole workspace, or nothing you can constrain: ask what topic or area the map should cover before reading anything.
...
Map the directory structure of the scope, then read the files that match it. Read each matching file far enough to extract its structure and ideas, its title, headings, frontmatter, and the opening of each section, without reading every file whole. When the scope is too large to read closely in full, and the request named a topic, read the files most relevant to that topic, judged by what they cover and not only by whether their path, title or headings contain its words, as many as this run can, and let Step 5 name what was left unread. The request named no topic words: ask which files to include, read those they name, and let Step 5 name any matching file they left out. No answer: do not write the map, and do not choose a subset by an unnamed standard. Skip binary and non-text files, which cannot be summarized.
...
Group ideas from across files by theme. Start from the natural groupings, a shared directory, overlapping headings, common keywords, and build a shallow hierarchy, broad topics over specific sub-topics, no more than three levels deep. A fourth level would be needed: place those ideas in the third level, and do not add a fourth. A topic must draw on at least two files: a grouping that would hold ideas from a single file is that file, not a topic, so leave it ungrouped. When the whole scope clusters into one topic, report a flat list under it rather than manufacturing sub-groupings. None of the groupings draws on two files: leave every grouping ungrouped.

### 5. Assemble and place

Assemble the map in the shape below. List files that were analyzed but fit no topic under the unmapped heading rather than dropping them, and name what the scope did not contain, including files left unread when the scope was too large. Then place the map as one new document in the owning root's work directory per `standards/conventions.md`, never at a root's top level and never in this plugin root. Where that standard names one home, place it there. Where more than one home is open, confirm the location with the requester before placing it, place the one they name, and do not place the file without an answer. No existing file is touched.
...
- **Ambiguous scope.** The request names nothing to constrain the analysis to, or names both a narrower scope and the whole workspace: ask before reading, per Step 1, not after producing a map of the wrong thing.
