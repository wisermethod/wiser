...
`gates.sh` is a sibling of this file and of `SKILL.md`, wherever this skill is installed; resolve it the same way the register companions beside a Content Author brief are resolved. It needs `bash` and the standard POSIX utilities: `awk`, `grep`, `sed`, `find`, `sort`, `cat`, `cut`, `tr`, `wc`, `head`, `uniq`, `basename`, `readlink`, `mktemp`, `mv` and `rm`. Without `readlink` every bound file reached through a symlink is refused as leaving the root, since that is how the containment check follows one. It is `bash` and not `sh`: the script uses process substitution, which `/bin/sh` rejects before any gate runs.
