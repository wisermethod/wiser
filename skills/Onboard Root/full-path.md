...
What each record is, where its type keeps it, and the written form every gate parses. The three sections below are that in order: where they go, what shape each takes, and the gate table that reads them.
