...
**At the minimum core**, where the Tier section retires the per-angle packages and G4 with them, this phase still produces one abbreviated research record. On a client root it sits at `work/onboarding/research-record.md`; on any other type it sits in that root's working area as `work/onboarding-research-record.md`, on the same pattern the second column of Where the records go, by root type uses, which carries no row for it because no gate reads it. It is not a per-angle package and G4 does not read it, carrying the angle, the retrieval mechanism, and what was retrieved with its sources and retrieval dates.
...
**A root is identified by the `type:` its own `AGENTS.md` declares, never by its folder name.** That type decides where this skill writes, because the templates declare different layouts. `gates.sh` uses that same `type:` and nothing else to decide where the records live, so the two never disagree about a path; where the table below and that script disagree about any path, they are both wrong until they agree again.
...
**Only the client template declares `work/onboarding/`, `sources/` and `todos/`.** The personal and org templates declare `work/`, `plays/`, `playbooks/`, `skills/`, `inbox/`, `zArchive/`, and `sites/`. The client template also declares `sites/`. A department or industry root made from the org template declares what it declares; the table above is where the mapping lives. Do not create empty `sites/`; Site Author creates the folder on first stand-up, and an onboarded root with no site is complete.

**Every phase in this file names the client path. Read each one through this table**, whichever type is being onboarded; the client column is what the phases say, and the other column is what they mean on the other types. The handover-shaped phases, meaning extraction and the supplied-originals inventory, exist only where documents were handed over. Per-angle evidence is not one of them: Phase 2 produces a package per angle from public research whether or not anything was handed over, and only the minimum core retires the packages, which is what the Tier section says and what G4's skip turns on. On a root where no documents were handed over, the handover-shaped phases do not apply, and `gates.sh` says so under the gate that reads them rather than failing it.
