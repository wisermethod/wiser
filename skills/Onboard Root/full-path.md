...
**Verification is a separate pass, and it is not optional.** No load-bearing claim enters a bound file until it has been read back against the source it cites, in a pass distinct from the one that produced it. A load-bearing claim is one a deliverable would act on: a prohibition, a compliance constraint, a commercial term, a named person's title or quote, a figure a deliverable would state, and the register decision. Reading back means opening the cited page, document, or file and locating that claim. A claim that cannot be located is not a free choice between dropping it and labeling it. Does a must-reach row require it to reach a file? Yes: do not drop it. It enters carrying `Unverified` with the failed search recorded, at the destination the forward check allows for its kind. No, and the claim is load-bearing, as this rule defines that: drop it from the bound file and record the failed search. It does not enter unlabeled. No, and it is not load-bearing, and a heading still needs the sentence: enter it labeled `Unverified`. No, and no heading needs it: drop it and record the failed search. This applies to a claim a sub-agent returned, a claim a supplied document states, and a claim you are confident about; confidence is the condition under which the rule is most necessary. Prevents: five critical errors that reached bound files as conclusions nobody re-read.

**Check the source against the file, not only the file against the source.** Every source document yields a must-reach list: its prohibitions, compliance flags, commercial terms, named people with titles, and internal review notes. Each item is shown to have reached a file, on the split the forward check states. A prohibition or a compliance flag reaches a bound file only. A commercial term, a named person, or a review note reaches a bound file, the operating file with a named disposition, or both. Read-back cannot see what was never written down. Prevents: a compliance prohibition and a cross-client leak that both sat in the source material and reached no bound file.
...
**Verified means located in the source, not true.** A source can be wrong, a draft, or authored by an interested party, which is why every extraction carries a provenance block. Where a claim's only support is one third party's unaudited assertion, is that assertion located in their document? Yes: it is verified-as-stated-by with that source named. It is not marked `Verified`, and it is not left unlabeled. No: it is `Unverified` with the failed search. It is not left unlabeled. Prevents: whole tables of one party's unaudited tool exports asserting measurement by carrying no label.
...
**Read what a source's own reviewers said about it.** Internal review notes, comments, speaker notes, and tracked changes carry the constraints and retractions the authors themselves flagged. Capture the proposal and the note. Where they conflict, what does the source show about the note? The note is dated later than the proposal, the source does not mark it superseded, and the source does not record it as rejected: carry the note as the candidate constraint and log the conflict. The note is dated earlier than the proposal, the source marks it superseded, or the source records it as one reviewer's rejected opinion: log the conflict, and carry the proposal as the candidate constraint with the note named as not governing. The source shows neither the note's date nor a supersession nor a rejection: log both and ask. Do not choose which governs. Prevents: extraction capturing what documents proposed and not what their own reviewers forbade.
...
2. The must-reach list per source, each item asserted on the standing rule's split: a prohibition or a compliance flag into a bound file only, and any other item into a bound file, the operating file with a named disposition, or both.
...
**Type.** Read `system/templates/AGENTS.md`; it names the one template, five types and their copy-time prompt sheets. Choose by the scope the new root will own, not by who asked for it. The discriminating question is who signs the outputs this root will hold, and whose facts and voice they carry; most requests answer it in one sentence.
...
- Department: `system/templates/AGENTS.md` owns the test for when a unit earns its own root. Is the parent org root in the workspace? Yes: copy the User Root Template, set `type: department`, and have `memory/about.md` name the parent organization first. No: ask whether to onboard the org first. Accepted: onboard that org root by this skill, and copy the department only once that org root is in the workspace. If it is still absent, stop as on a decline. Declined: stop. Do not copy the department. Record the declined answer in what you tell the person. There is no department root to mark onboarded. Do not copy the department while the org root is absent.

**Real name.** Roots carry the real name the people involved use, the person's own name or the organization's short name, never a type name and never a template's name; a folder called `Client` leaves the next reader asking which client. Check the chosen name against the Names rules in `standards/conventions.md`, including the case rule, against the roots already in the workspace. Does the chosen name pass? It passes, and the request gives one spelling: use it. It passes, and the request gives a shorthand, an initialism, or a form that could be written more than one way: confirm the exact spelling before copying. It fails: report the rule it breaks and ask for a name that passes. Do not rename it on sight, which is what that standard yields here, and do not copy under the failing name.
...
**The subject-consent question.** Is the subject a person, or will the root hold personal data? No: write `consent: not-applicable: <reason>` on the run record. Yes: has the subject agreed to be researched and recorded, and did they name anything not to gather or keep? Write that answer on the run record's `consent:` line. Agreed, and they named nothing to exclude: research may include them. Agreed, and they named exclusions: do not gather or keep what they named. The subject has not been asked, or the answer is not known: say what will be gathered before gathering it, keep it to what the work needs, and record that consent was not established. Refused: do not research that subject and do not write them down from research. Research that is not about that subject may still run. Searching a person and writing them down is not sanctioned by the requester asking for it.

**The competitors offer.** Reasoning, not evidence: no recorded failure supports it. Offer it for every client root. For an org root, does the request ask for a competitive set on record? Yes: offer it. No: do not offer it, and record `not-offered: org`. For a personal, department, or industry root, did the requester ask for the offer? Yes: offer it. No: do not offer it, and record `not-offered:` with that type. An industry `about.md` already describes the field.
...
**Copy the template.** The template's `AGENTS.md` ships a `## Onboarding` section saying the root is not onboarded and every key blocked. It stays that way until Phase 9 rewrites it, and this file is the authority on the sequence between.
...
- Copy `system/templates/User Root Template/` whole, hidden files included, so the declared empty directories survive, then confirm the copy holds the same file names as the source. Set `type:` to Phase 0's type and `root:` to the chosen name. Copy the matching `prompts/<type>/about.md` and `design.md` onto the copied memory files. Remove the copied `prompts/` after selecting the sheets and replace `[name]` throughout the remaining copy. Department and industry use this same tree; their first about line names the parent organization or field. Record copy vantages before these intended transformations, and verify the transformations separately.
- After those transformations, read `standards/user-root.md` C1 and state the expected integer in `work/onboarding/verification.md`: "This copy must carry `layout: 3`, the current tree version read from `standards/user-root.md` C1." Use the integer actually read if C1 has advanced. Compare the copied frontmatter declaration to that integer before Phase 1 exits. Does the copy carry exactly one `layout:` integer, equal to the integer just read? Yes: Phase 1 may exit this check. No: make the frontmatter carry exactly one `layout:` set to that integer, then compare again. Do not continue while it is missing, malformed, duplicated, or different. Inherited template bytes alone are not evidence. Run G21 again with the ordinary harness at Phase 9.
...
**Every type uses the common tree** in `standards/user-root.md`. Copy only into an empty destination. Records follow C10, supplied originals C5, and first-use directories C3. Competitors is created from a confirmed set only; the unified template does not carry an unbound competitors stub.
...
**At the minimum core**, where the Tier section retires the per-angle packages and G4 with them, this phase still produces one abbreviated research record. For every type it sits at `work/onboarding/research-record.md`, per `standards/user-root.md` C10. It is not a per-angle package and G4 does not read it, carrying the angle, the retrieval mechanism, and what was retrieved with its sources and retrieval dates.

**Sub-agents, at full tier.** Spawn one per angle, **all of them in one batch rather than one after another**. The angles do not read each other and the stopping rule is evaluated on what comes back, so running them in sequence buys nothing and is most of the wall-clock time this phase costs. Where the host cannot run them concurrently, say so in the close report. The brief is: **return evidence, not conclusions.** Before any of a returned package is used, what does the check show? The four required elements are present, and three entries spot-checked against their cited source match it, with those outcomes recorded in the package: the package may be used. An element is missing, the package is conclusions rather than evidence, or a spot-check does not match its cited source: record the outcome, reject the package, and re-brief it. It is not edited into memory, and its conclusions are not mined for claims.
...
**Every row's outcome is one of three**: located, located-elsewhere-and-citation-corrected, or not-located. A not-located claim follows the four outcomes in the standing rule on a separate pass, and the failed search is recorded in its row. It is not a free choice between dropping the claim and labeling it.
...
**The forward check starts here and closes at Phase 9.** Walk each must-reach list into the draft, and record for every item the bound file and anchor it is going to reach, or the operating item that will own it. The dispositions are written here; they are re-run against the written files at Phase 9, because a disposition naming a location is a promise until that location exists. Which kind is the must-reach item? A prohibition or a compliance item reaches a bound file only: downstream work loads `memory/`, it does not load the todo list, so a constraint discharged to `work/onboarding/operating-file.md` is absent from everything a deliverable reads. An operating item may sit alongside that bound sentence and may not replace it. A commercial term, a named person, or a review note may reach the bound file, the operating file with a named disposition, or both. Did a sentence for it survive verification? Yes: it goes in the bound file, and an operating item beside it is permitted. No: it goes in the operating file with a named disposition. Do not invent a bound sentence, and do not drop it.
...
- **Never fill a heading from what you already know about a named person or company.** That is a research inference at best, and an unsourced person-fact does not enter the file at all. What supports this heading? A claim that survived Phase 3: write it, with its anchor. A source states it and Phase 3 has not checked it: verify it before it enters. Nothing in the sources states it, and the person can be asked: ask. Nothing states it, and it was asked with no answer or cannot be asked this run: mark the heading unanswered with one of the standard's four labels.
- **Both readings, every time, or a named decision with the decider.** Do two sources give different answers for this claim? No: write the one that survived. Yes, and a named person has chosen: record both readings and who chose. Yes, and nobody has chosen: record both readings, name that nobody has chosen, and carry the choice to Phase 5. Do not drop one, and do not choose. A choice that is silent is unrecoverable.
...
**Credential bindings** follow the constitution's Workspace Model, Secrets. The unified template supplies no secrets home; follow that declaration contract rather than deriving a credential location from type.
...
2. Write the file at the path the binding names, one bare `KEY=` per line, values empty, nothing else in the file. Add the binding where the template directs. A personal root needs no binding: the constitution's Secrets rule resolves the key there by its own default, and the file is named for the platform.
...
- **What was actually bought, on a client root.** Fee, term, volumes, cadence, and which separately priced items are in scope. **On the other types this class is what this root is for**: what work it exists to carry, who relies on it, and what falls outside it. A personal root is not a purchased engagement and is never asked what it cost.
- **Who confirms what, and on what basis.** The named person who signs off on facts, on voice, and on anything leaving this root, and what makes them competent to do so, per domain. Does the named basis show the person is competent for that domain? Yes: record the name and the basis. The person is named and the basis does not show competence for a compliance-relevant sign-off: record the name, record that the basis does not cover it, and name whose sign-off is still needed. Do not close that key complete on this sign-off alone.
...
`memory/voice.md` is authored by `skills/Build Voice/`, not here. Hand the requester to it by name, with the root's path, its declared type, the evidence packages, and the answer to what the outputs are for. Build Voice owns what goes in the file; this skill owns the run until close. Build Voice's confirmation sample is Ghost-Writer-gated as prose before the authority sees it, per `skills/Build Voice/SKILL.md` step 10. What did Build Voice return? Its own Success met: close the voice key complete. A usable file, stopped or unconfirmed: close the key provisional and scope the deliverable-write refusal to voice-dependent work rather than holding the root. No usable file: close the key blocked and scope the refusal the same way. On that blocked line only, did a named person refuse the writing or the handoff? Yes: the blocker is `person:` that person. No: the blocker is `capability:` Build Voice. The owner on either open line is the person named under who-confirms for voice. If that class was deferred, use the named person or role on its operating-file row. If neither names one, ask for a person or a role before writing the state line. Do not write the owner as the requester, and do not choose provisional or blocked by preference. Text no confirming authority accepted clears nothing; report the state rather than writing a voice yourself.
...
`memory/design.md` is how this root's outputs look, documents, decks, and images, where `memory/voice.md` is how they sound; presentation-producing skills read it. Unlike about and voice it does not block onboarding. Did the requester accept the offer to do design? Yes: take one of the three sources below, and Phase 9 scores the key complete, provisional, or blocked like any other key. No: replace each prompt line with `Not available` and the reason that design was declined, and continue. The owner on the state line is the person named under who-confirms for design, or for facts if design names none. If that class was deferred, use the named person or role on its operating-file row. If neither names one, ask for a person or a role before writing the state line. Phase 9 scores that key. Do not stop the onboard, do not leave a prompt line in place, and do not write the owner as the requester.
...
`gates.sh` is a sibling of this file and of `SKILL.md`, wherever this skill is installed; resolve it the same way `skills/Content Author/` resolves the content-type file it loads, as a sibling of that skill's own `SKILL.md`. It needs `bash` and the standard POSIX utilities: `awk`, `grep`, `sed`, `find`, `sort`, `cat`, `cut`, `tr`, `wc`, `head`, `uniq`, `basename`, `readlink`, `mktemp`, `mv` and `rm`. Without `readlink` a bound file whose own name is a symlink is refused as one that cannot be resolved, since that is how the containment check follows a leaf; a path reached through a symlinked directory needs none of it, because those hops resolve through `cd -P`. It is `bash` and not `sh`: the script uses process substitution, which `/bin/sh` rejects before any gate runs.
...
Copy still-open root-meta items to `zBuilds/builds.md` under `standards/user-root.md` C8. Name still-open delivery items and their owners in the relevant workstream `AGENTS.md`; link the originating operating item so the handover is traceable.
...
What each record is, where its type keeps it, and the written form every gate parses. Where they go is below, then the shape each takes; the gate table that reads them is the section after this one.

### Where the records go, by root type

`standards/user-root.md` C10 owns record placement for all five types. Type decides close intensity, not paths. The personal short path does not produce a run record, close report or operating file. `gates.sh` uses the same fixed resolver; a declared alternative records home is an unsupported change to file as a build.

| Record | Every type |
|--------|------------|
| Run record | `work/onboarding/run-record.md` |
| Verification | `work/onboarding/verification.md` |
| Audit | `work/onboarding/audit.md` |
| Operating file | `work/onboarding/operating-file.md` |
| Close report | `work/onboarding/close-report.md` |
| Extraction records | `work/onboarding/extraction/` |
| Evidence packages | `work/onboarding/evidence/` |
| Working draft | `work/onboarding/draft/` |
| Supplied originals | `sources/` |

Inbox remains the unclassified drop zone under `standards/user-root.md` C5. Only phases that run create records. Extraction and supplied-originals inventory require supplied documents; public research evidence does not. The Tier section governs minimum-core retirement of per-angle packages and G4. The template declares first-use sites under C9 and never creates an empty site directory.

### The written form of each record

Load `record-grammars.md` before producing any record; its tables and key lines are what the gates parse.

A gate cannot run against a record with no shape. These are the shapes, and they are what makes the close mechanical rather than a matter of opinion. `system/templates/User Root Template/` ships the marked headings in `memory/` and, under `work/onboarding/`, its `AGENTS.md` and nothing else; that router points here, and `record-grammars.md` beside this file holds the full schema of every record, and the run writes each record itself. `standards/conventions.md` still owns what a register and a label mean; what follows is only their written form.
...
**The placeholder tokens** are `[name]` in every template, and `[Competitor]` if a competitors draft introduced that placeholder. Greppable as `\[name\]|\[Competitor\]`.
...
Twenty-seven checks. Run them at Phase 9, per key. Each names what runs and what makes it fail; all paths are relative to the new root. A phase exit runs its gates against the files that phase wrote; only Phase 9 runs them across the whole tree.
...
| G7 Deferrals are named | Every `work/onboarding/operating-file.md` row with status gating or blocking has a blocker matching `^(person\|credential\|capability): .+` and a non-empty attempt | A category-only blocker such as "a human", or a blocker with no recorded attempt |
...
| G11 Label vocabulary is closed | Extract every bracketed token in `memory/` matching `\[[A-Z][A-Za-z ]*(:[^]]*)?\]`, take the part before any colon, diff against `Verified`, `Estimated`, `Unverified`, `Not available`. Anchors are excluded by the pattern; `[Competitor]` is exempt wherever it appears, and G18 is what refuses one that survives | Any other token. This is what catches an invented vocabulary mechanically |
| G11b Figure tables carry provenance | Every table a figures heading opens, which is a heading whose own text names figures or one carrying or immediately followed by a `<!-- figures -->` marker, and every table anywhere in a bound file that carries a `Figure` column whatever heading it sits under, carries the columns Figure, Unit, Denominator, Tool, Window, Label, Source, every data cell non-empty | A missing column or an empty cell. This is what catches a table of figures asserting measurement, and the denominator half of destructive summarizing |
...
| G16 Findings disposed, disputes preserved | Every findings row names its deciding check. Every rejected row names at least one place the claim was looked for, which the gate takes as a container, a URL, a file or an evidence row; naming the check without naming a place is not a search. The standing rule that a claim is searched in every container the format has is the run's obligation, and it is wider than what a gate can mechanically count. Every disputed row names a bound file that exists and contains `(Disputed:` with that finding id | A finding with no deciding check, a rejection naming no place it was looked for, or a dispute that never reached the file. This is what stops rejection being cheaper than fixing |
| G17 The close is in the root | `AGENTS.md` carries `## Onboarding` with a `- <key>: <state>` line for every key `run-record.md` closes; each state equals the run record's, and a provisional or blocked line says what it waits on. | The section is absent or still reads as the template shipped it, a key line is missing, the two disagree, an open key names no owner or blocker, the run record is missing or closes a key with a word the grammar does not carry, or the record is ambiguous: a repeated heading; a key repeated inside the span that declares it; a key the preamble or `## Copy vantages` declares but answers outside it instead, and for `## Copy vantages` also one answered earlier in the file with a different value, which is the answer `kv` reads first; or a fence, an HTML comment, raw HTML, a blockquote or a line indented four columns, anywhere in the record and whether or not a declaration sits inside it. `## Per-key close` is checked for repeats alone. |
...
| G19 Paths and credentials resolve | Every path-shaped token in every bound file, not only those Provides binds, resolves to an existing file or directory inside this root. A path-shaped token carries no whitespace, backticked or not, and sits under `memory/`, `work/`, `sources/`, `todos/`, `inbox/` or `zArchive/`: a name carrying a space is not read here at all, and backticking it does not make it read, so a citation of a supplied file whose name has spaces is not checked by this gate. Plus, per credential file: `grep -cE '^[A-Za-z_][A-Za-z0-9_]*=' <path>` counts key lines and `grep -cE '^[A-Za-z_][A-Za-z0-9_]*=[[:space:]]*$' <path>` counts empty values, and on the personal path a third `grep` over the same file takes the names of the empty-valued keys so the note can say which credential is unowned. A value is scanned to tell an empty one from a filled one and is never printed; only key names leave the file | A path that leaves the root, a path that does not resolve, zero key lines, or empty values above zero with no `person:`, `credential:` or `capability:` blocker naming that credential anywhere in the operating file the root's own type keeps, `work/onboarding/operating-file.md` for every type. An unfilled credential file is legitimate; an unfilled credential file nobody owns is not. The personal path keeps no operating file at all, so there the unowned keys are named as a note rather than failed. Hygiene only |
| G20 Operating file and close report | Every `work/onboarding/operating-file.md` row has an owner that is not "the requester", "requester", "the client", "TBD", "whoever asked", or empty; a status in the four; and, for done, a result carrying a date. `close-report.md` carries all eight headings, each non-empty | An unowned gap, a status outside the four, a done row with no dated result, or an empty close-report heading |
| G21 Layout stamp agrees with C1 | A declared Wiser user root (recognized type, Provides, Wiser constitution citation) carries one frontmatter `layout:` integer matching the current tree version read from `standards/user-root.md` C1. Runs on the personal short path too; blocked keys do not affect it. PASS proves stamp agreement, not every C-clause. Undeclared roots SKIP with the missing declaration reason; SKIP is not a pass and exits 1 | Missing, malformed, duplicate, or disagreeing layout; missing, ambiguous, or malformed C1 authority |
...
- **Closing a key in the run record and not in `AGENTS.md`.** The `## Onboarding` state lines are what a later session reads before a deliverable write. A run record that says provisional beside a root file that says complete lets through the write the close refused. G17 compares the two.
