...
| G19 Paths and credentials resolve | Every path-shaped token in every bound file, not only those Provides binds, resolves to an existing file or directory inside this root. A path-shaped token carries no whitespace, backticked or not, and sits under `memory/`, `work/`, `sources/`, `todos/`, `inbox/` or `zArchive/`: a name carrying a space is not read here at all, and backticking it does not make it read, so a citation of a supplied file whose name has spaces is not checked by this gate. Plus, per credential file, and never by opening it: `grep -cE '^[A-Za-z_][A-Za-z0-9_]*=' <path>` counts key lines and `grep -cE '^[A-Za-z_][A-Za-z0-9_]*=[[:space:]]*$' <path>` counts empty values. Both print numbers and nothing from the file | A path that leaves the root, a path that does not resolve, zero key lines, or empty values above zero with no row naming that credential in the operating file the root's own type keeps, `todos/current.md` on a client root and `work/onboarding-operating-file.md` on the others. An unfilled credential file is legitimate; an unfilled credential file nobody owns is not. The personal path keeps no operating file at all, so there the unowned values are named as a note rather than failed. Hygiene only |
