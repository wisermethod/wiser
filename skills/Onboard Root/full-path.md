...
| G17 The close is in the root | `AGENTS.md` carries `## Onboarding` with a `- <key>: <state>` line for every key `run-record.md` closes; each state equals the run record's, and a provisional or blocked line says what it waits on. Fails when the section is absent, a key line is missing, the two disagree, or an open key names no blocker. |
