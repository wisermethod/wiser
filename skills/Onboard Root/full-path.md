...
| G19 Paths and credentials resolve | Every path-shaped token in every bound file, not only those Provides binds, resolves to an existing file or directory inside this root. A path-shaped token is a backticked or whitespace-free one under `memory/`, `work/`, `sources/`, `todos/`, `inbox/` or `zArchive/`: a name carrying a space is not read, which is why a supplied file whose name has spaces is cited backticked. Plus, per credential file, and never by opening it: `grep -cE '^[A-Za-z_][A-Za-z0-9_]*=' <path>` counts key lines and `grep -cE '^[A-Za-z_][A-Za-z0-9_]*=[[:space:]]*$' <path>` counts empty values. Both print numbers and nothing from the file | A path that leaves the root, a path that does not resolve, zero key lines, or empty values above zero with no `todos/current.md` row naming that credential. An unfilled credential file is legitimate; an unfilled credential file nobody owns is not. Hygiene only |
