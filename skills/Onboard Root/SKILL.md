...
|------|-----------|-----------|
| G0 The substantive floor | For every key closing complete, its required classes each have at least one row in `verification.md` `## Claims` with outcome located or located-elsewhere-and-citation-corrected, whose anchor appears in the `memory/` file that row names. Required classes: `about` needs who-confirms and hard-constraints, plus what-was-bought on a client root and what-this-root-is-for on the other four; `voice` needs register-decision, register-confirmation; `design` needs design-source; `competitors` needs set, set-confirmed-by, set-date | A key is marked complete and any required class has no located row. An unbound `competitors` key is skipped where `competitors-offer` records a declined or deferred offer, and failed where it records an accepted one or no offer at all. **A `competitors` key closing complete needs exactly one binding of the key to `memory/competitors.md` in the Provides block, that file to carry content of its own beyond its scaffolding, and all three competitor classes to anchor in that file specifically**, because a set recorded anywhere else is not where downstream work resolves it. **Any heading or key line appearing twice in the run record fails, and G17 is the gate that refuses it**: every consumer would otherwise pick an occurrence, and whichever it picked would be a value some other consumer never saw. This covers `###` headings as well as `##` sections, and key lines as well as headings, because `section_body` merges every occurrence of a heading while `kv` returns the first of a key. This is what stops a root of `[Not available]` headings closing complete |
...
| G17 Refusal removed only for complete keys | Where every key is complete, or unbound against a declined or deferred offer, `grep -n '## Instantiation' AGENTS.md` prints nothing. `unbound` is refused for any key but competitors, and for competitors where the offer was accepted or never made. Where any key is provisional or blocked, that section is present and names each such key | The section deleted while a key is provisional or blocked, or retained naming no key. Hygiene only |
