...
Which property, and which account reaches it, are inputs too. Access is through the gateway's `google` / `search-console` and `google` / `analytics` grants. These are two grants; one never serves both. Could more than one account reach this property, on what the requester has said? Yes: ask which. Do not pull until they answer. No answer: do not pull. No, or they have not said: do not ask. Availability is what `execute` returns, never a credential file.
...
**Head elements.** A title is written to be read in a result list, not to hold a keyword: the term appears once, early where the sentence allows it, never forced. Around sixty characters for a title, and a hundred and twenty to a hundred and sixty for a description, are display conventions rather than published limits, so the count travels beside every element. Does the count sit inside that convention? Yes: the count beside the element is enough. No: keep the wording and state the crossing beside the count. Do not treat the crossing as an error. Each is unique against the rest of the site, and each description ends in something specific to do. Does the current element already meet every requirement in this paragraph: the term appears once, it sits early where the sentence allows it and is not forced where the sentence does not, the element is unique against the rest of the site, and the description ends in something specific to do? Yes: return it unchanged and say so. No: rewrite the requirement it fails. You cannot tell whether it is unique, or whether the term is forced: do not return it unchanged. Ask. No answer: do not return it unchanged, and do not ship that element as already meeting the requirements.
...
**Answer-engine surface.** List only canonical, indexable pages, each with the one thing it covers; a staging address, a redirect, or a line reading "our blog" makes the file worse than absent. The description block is what a generated answer will repeat, so it states what the thing is before it says anything persuasive. Did the requester decide the directive for this crawler? Yes: write that decision. No: ask. Do not copy a default from another site. No answer: omit that crawler and say the directive was not decided.

**Redirect map.** Did the requester name a reason for a temporary redirect? Yes: use the temporary status they named. No: the redirect is permanent. The old address is the exact path served. Which live page covers what the old address covered? One: that page is the target. Two or more cover it equally: ask which. No answer: list the old address with none and say they tied. None covers it: list it with none and say it has none. A blanket redirect to the homepage is not a target in any of these outcomes. Chains are collapsed to their endpoint and loops are named.

**Content brief.** Does the query admit one intent? Yes: settle on it, then write the brief. More than one: ask which intent before writing. Do not write a brief that tries to satisfy both. You cannot tell: ask before Step 2. No answer: do not write the brief. One written over an ambiguous query produces a page that satisfies neither reading. Format and length are calibrated from the pages actually ranking for the query, never from a word-count rule; where no source in Evidence returns them, that reading is taken by hand and carries `Estimated: manual review`. Separate what every ranking page covers from what none of them does, since the second is the only reason the new page wins.
...
Name the one site, the artifacts wanted, and where their order came from.

Does the request name more than one site to produce artifacts for? Yes: stop. That is two runs. A competitor named as the thing this site is measured against is evidence rather than a second subject, and stays in one run. No: one site.

Does the request name what the artifact should fix, in `<findings>` or from the requester? Yes: use that, and the run resumes with it. No: is the artifact one whose scope a reading settles without choosing what to change? An `llms.txt` over the pages the site publishes as canonical, or a tracker report between two snapshots, are the ordinary cases: proceed without findings. Another artifact whose row is filled by a reading the run takes, with no page chosen and no order chosen: proceed the same way. An artifact that needs pages, a priority, or a fix chosen, and neither the findings nor the requester chose them: go to `experts/Webmaster/` and come back with findings. Do not produce it yet. "Write me some meta descriptions" is that case: which pages carry the problem, and why those, is what that expert answers. You cannot tell which of these it is: ask. Do not produce it yet.

Does a plan file for this site already exist? No: start from the request. Yes: take the first match that fits, and do not also take a later one. The request asks for a fresh set: archive that file per `standards/conventions.md`. Do not edit it. Start the new set. You cannot tell whether the request is a fresh set: ask. Do not archive and do not edit the file. The request names one artifact: serve that artifact on its own. It implements an entry in the file: record it against that entry. It implements none: serve it and do not add an entry. The request does not ask for a fresh set and does not name one artifact: read the file before anything else and continue from the first entry that is not live. Do not start the set over.

Then settle four things before any evidence is pulled, because each decides what can be produced at all. Does the envelope have `site/kit.json`? Yes: a kit write is permitted. Only a domain-folder `kit.json`: Site Author Wrap is required before that write. Neither: foreign paste-ready files. Can anyone change the site's code? Yes: say so, and continue. No: say so before any artifact is written, and continue. An artifact nobody can deploy is still written. You cannot tell: ask before any pull. No answer: do not pull and do not write yet. Where is a redirect or a head element implemented: on a server, at the edge, or inside a content platform? One of those: say which. Step 3 writes those artifacts for that place. More than one, or none is stated: ask before any pull. No answer: do not pull and do not write yet. Which login reaches the property is Inputs' question.
...
Is a reading missing that this artifact's row cannot be carried without, so a thinner file would read as complete? Yes: say which artifact cannot be built and why. Do not produce it. No: label the missing reading per `standards/conventions.md` and build from what arrived.
...
Did the evidence supply this value? Yes: use it. No: ask the requester for it. They answer: use the answer. They decline, or they do not answer: leave it out and name it missing. Never fill it, per the evidence labels in `standards/conventions.md`: a placeholder that survives into a delivered artifact is published as though someone meant it.
...
Before the hand-over, the gate: hand each finished artifact, the finding it was built to close, which stands as the goal, or its row in the artifacts table where its own evidence settled the scope, and the evidence it rests on, never the reasoning that produced it, to `experts/Webmaster/` in a second context. What did it return for this artifact? Pass: hand it over. Return, with what fails: go back to Step 3. The requester declined the review: name the decline in the hand-over. Neither a verdict nor a decline: do not hand it over as ready.
...
For a custom `site/public/llms.txt`, inspect the kit's generated `/llms.txt` route. Does the built or served response match the intended artifact? Yes: it may be declared ready. A route collision, or a response still generated from the old content: return that artifact for code integration outside this skill. Report the unverified surface and leave the kit route unchanged. You cannot fetch a built or served response: do not declare it ready. Report the unverified surface and leave the kit route unchanged. A file write alone does not prove the custom text will be served.
...
1. This skill writes; it does not rank. Does the artifact need a priority order? No: do not invent one. Yes: did `<findings>` or the requester supply one? Yes: use it. No: ask. Is the answer a preference the requester states as their own choice? Yes: use it. Is the answer a judgment about what the site should change, or does the requester want that decided? Route the question to `experts/Webmaster/`. No answer: do not create an order, and do not produce the artifact as if it had one.
