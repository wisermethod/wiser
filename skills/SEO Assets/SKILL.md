...
Which property, and which account reaches it, are inputs too. Access is through the gateway's `google` / `search-console` and `google` / `analytics` grants. These are two grants; one never serves both. Where the requester has said more than one account could reach the property, ask which, and do not pull until they answer. Where they have not said that, do not ask. Availability is what `execute` returns, never a credential file.
...
**Answer-engine surface.** List only canonical, indexable pages, each with the one thing it covers; a staging address, a redirect, or a line reading "our blog" makes the file worse than absent. The description block is what a generated answer will repeat, so it states what the thing is before it says anything persuasive. Crawler directives are written as a decision the requester made about each crawler, never as a default copied from another site. Where they have not decided one, ask. Where there is no answer, omit that crawler and say the directive was not decided.

**Redirect map.** Permanent unless the requester names a reason for a temporary one, in which case use the temporary status they named. The old address is the exact path served, and the target is the one live page that covers what the old address covered. Where two or more cover it equally, ask which, and where there is no answer list the old address with none and say they tied. An address with no equivalent is listed with none and said to have none. A blanket redirect to the homepage is not a target. Chains are collapsed to their endpoint and loops are named.

**Content brief.** Settle the intent before writing the brief. Where the query admits one intent, settle on it. Where it admits more than one, ask which intent before writing, and do not write a brief that tries to satisfy both. Where you cannot tell, ask before Step 2. Where there is no answer, do not write the brief. One written over an ambiguous query produces a page that satisfies neither reading. Format and length are calibrated from the pages actually ranking for the query, never from a word-count rule; where no source in Evidence returns them, that reading is taken by hand and carries `Estimated: manual review`. Separate what every ranking page covers from what none of them does, since the second is the only reason the new page wins.
...
One names more than one site to produce artifacts for, which is two runs. A competitor named as the thing this site is measured against is evidence rather than a second subject, and stays in one run.

The other names an artifact but not what it should fix. Where the request names what the artifact should fix, in `<findings>` or from the requester, use that, and the run resumes with it. Where it does not, an artifact whose scope a reading settles without choosing what to change proceeds without findings: an `llms.txt` over the pages the site publishes as canonical, a tracker report between two snapshots, or another artifact whose row is filled by a reading the run takes, with no page chosen and no order chosen. An artifact that needs pages, a priority, or a fix chosen, and neither the findings nor the requester chose them, goes to `experts/Webmaster/` and comes back with findings before it is produced. "Write me some meta descriptions" is that case: which pages carry the problem, and why those, is what that expert answers. Where you cannot tell which of these it is, ask, and do not produce it yet.

Where a plan file for this site already exists, take the first match and not a later one. A request for a fresh set archives that file per `standards/conventions.md` and starts the new set; do not edit it. Where you cannot tell whether the request is a fresh set, ask, and do not archive or edit the file. A request naming one artifact is served on its own, recorded against that file's entry where it implements one, and not given a new entry where it implements none. Where the request is neither a fresh set nor one named artifact, read the file before anything else and continue from its first entry that is not live rather than starting the set over. Where no plan file exists, start from the request.

Then settle four things before any evidence is pulled, because each decides what can be produced at all: `site/kit.json` permits a kit write, only a domain-folder `kit.json` requires Site Author Wrap before that write, and neither means foreign paste-ready files. Say whether anyone can change the site's code before any artifact is written; an artifact nobody can deploy is still written. Where you cannot tell, ask before any pull, and do not pull or write without an answer. Say where a redirect or a head element is implemented, on a server, at the edge, or inside a content platform, because Step 3 writes those artifacts for that place. Where more than one applies, or none is stated, ask before any pull, and do not pull or write without an answer. Which login reaches the property is Inputs' question.
...
An artifact missing a reading its row cannot be carried without, so that a thinner file would read as complete, is a different case: say which artifact cannot be built and why, and do not produce it. Otherwise label the missing reading per `standards/conventions.md` and build from what arrived.
...
A value the evidence did not supply is asked for. Use the answer. Where they decline, or they do not answer, leave it out and name it missing. Never fill it, per the evidence labels in `standards/conventions.md`: a placeholder that survives into a delivered artifact is published as though someone meant it.
...
Before the hand-over, the gate: hand each finished artifact, the finding it was built to close, which stands as the goal, or its row in the artifacts table where its own evidence settled the scope, and the evidence it rests on, never the reasoning that produced it, to `experts/Webmaster/` in a second context. It returns a verdict per artifact: a pass is handed over, and a return, with what fails, goes back to Step 3. A review the requester declined is named in the hand-over. Neither a verdict nor a decline, and it is not handed over as ready.
...
1. This skill writes; it does not rank. An artifact whose content depends on a priority order takes that order from `<findings>` or from the requester. Where neither supplies one and the artifact needs one, ask. A preference the requester states as their own choice is the order. A judgment about what the site should change, or a requester who wants that decided, is routed to `experts/Webmaster/`. Where there is no answer, do not create an order, and do not produce the artifact as if it had one.
