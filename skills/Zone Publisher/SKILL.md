...
| A provider's standing records, such as MX and SPF sets | That provider's current documentation |

A guess here fails silently: mail keeps flowing while it is unsigned, a verification quietly lapses, a DMARC policy rejects mail nobody meant to reject. Does the change need a value from this table? No: leave the pull's record as it is. Yes: which row applies, and is its source in hand? The row names one source and that value is in hand: use it. The row names one source and the value is not in hand: ask. The row names the live record and another source. Is the request replacing the value? No, and the pull has it: use the live record. No, and the pull does not have it, and the other source is in hand: use that source. Yes, and the other source is in hand: use that source. Yes, and it is not in hand: ask. The standing-record row: is that provider's current documentation in hand? Yes: use what it states. No: ask. There is no safe DMARC default. The answer names the value: use it. The answer is not available: leave that record out and say which one is missing. Do not invent a value, and do not publish a placeholder.
...
**4. Diff intended against live.** Three lists, built from the file against step 2's record objects. No live list from step 2: do not diff, and do not publish. For a type other than MX, match on type and name. For MX, how many live records share that name, and how many does the file have? One live and one in the file: match on type and name, and a different priority is a Change. One live and more than one in the file: ask. Do not pair two file records to the same live record, and do not publish. More than one live: match on type, name, and priority, so a different priority is one Remove and one Create. None live, and the file has one or more: each file record is a Create.
