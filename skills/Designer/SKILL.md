...
Five phases in order. Has the request, or the bound `design` file, already fixed this phase's output? The whole phase: state it as settled and skip it. Part of it: state the fixed part as settled and run only what is not fixed. Nothing in it: run the phase. Do not re-run a part that is already fixed.

### 1. Brief

Understand what is being designed before any visual decision. Four things are required and everything else is inferred.

- **The deliverable.** A specific output, not a goal: "a pricing page", not "improve the site". Does the request name one specific output? Yes, and a skill in this root produces it: that is the deliverable. Yes, and no skill in this root produces it: name it as unproduced when Phase 4 is reached, and do not approximate it. No: offer the choice between one component, one page, a system, and a set of screens. They confirm one: continue with that. They name a different output this root produces: continue with that. No answer: do not continue.
...
How many of the four required items above does the request leave unset? None: summarize the brief in a few lines and confirm it before designing. One to three: ask those, and no others, each carrying a recommended default drawn from the request. More than three questions would be needed: ask only the deliverable, the audience, and the job, each with a recommended default, infer the constraints, and state every inference. The user corrects anything: update the brief and confirm again. No answer, or a correction that leaves one of the four unset: do not design. The brief is a working document, not a deliverable, and is not saved as a file unless the user asks for one.
...
- **Subject anchor.** The one concrete subject, its audience, the single job of the artifact, and what in the subject's own world the design can draw from. Does the confirmed brief name all four? Yes: state them. One or more missing, and the brief supports an inference for each missing one: state the inference and say it was inferred. A missing one the brief does not support: ask for that one before stating the direction. Do not invent a subject, an audience, a job, or a world the brief does not supply.
...
- **Signature.** The single element this design will be remembered by, and the one place boldness is spent, with everything around it quiet and disciplined. Does the brief justify one aesthetic risk? Yes: that risk is the signature. No: still name one signature, state that the brief did not justify it, and confirm it with the direction before proceeding. Do not ship a direction with no signature. A design with no memorable anchor reads as templated, so declining the risk is itself a risk.

Does a live site or a codebase already carry a direction that shipped and was never written down? Yes: read it back first (Reference: Brand extraction) and let what it returns stand as the direction, and as Phase 3's existing tokens. No: state the five things in this phase. Either way, state the direction before proceeding. Does the request name a direction that matches the one you are about to use? Yes: proceed. It names none, or it contradicts the extraction: confirm before proceeding.
...
- **Intentional Choice Check.** Point at any element and state its job in one phrase and the reason behind its value. "It looks nice", "it is modern", and "standard practice" are not reasons. Can you state the job and a reason that is none of those three? Yes: keep the element and record both. No: redesign the value until you can. A redesign still cannot, and the brief does not require the element: remove it. The brief requires the element: keep it, and name the value that still has no stated reason as a finding for the review below. Do not remove it.
- **Distinctiveness Check.** Scan the artifact against the Prohibited Defaults Taxonomy in `experts/Creative Director/EXPERT.md`, the list's single home; it is dated and it changes, so read it rather than working from memory, and replace each match with an intentional alternative. A match the brief fixed or Phase 2 chose as the justified signature is exempt: a deliberate, stated choice on an axis the brief or direction settled is a decision, not a default, and the check records the justification instead of replacing the choice. Then run the same-prompt test on the whole direction: work through a generic version of this brief and see where it lands. Does that generic version land in the same palette, the same type pairing, and the same layout as this direction? All three: the direction is a default. Revise it, and state what changed and why. One or two: revise each part that matches, keep what differs, and state what changed and why. None: this test passes. Where the artifact documents or extends a system that already shipped, a match inside that system is reported to its owner as a finding rather than replaced: a deliverable that documents a system never quietly rewrites it.
...
Hand a UI or page artifact to `experts/Creative Director/` for a verdict, with Phase 1's brief plus Phase 2's direction and register as its `<brief>`, so any axis a deliberate choice already fixed is judged as fixed. What came back? Findings: work them before delivering. Does a finding reorder or replace a region so another element's place would no longer exist as composed? Yes: send the affected work back through Phase 4 and re-run this phase. No: work the finding in place. A verdict with no findings: deliver. The requester declines the review: deliver and say so. The expert is absent from the workspace: say the review degraded and stand on the three checks above. Where a Phase 4 skill already ran its own pre-ship design review, work those findings first so the expert judges a revised artifact rather than a draft, and never run the same review twice.
...
Four moves. Inventory the content and rank it in tiers: what the page's job requires, what supports it, what can sit lower. Which structural pattern does the page's job match? Content or a form: a single column. A sidebar beside a main area: two columns. A collection: a grid. A narrative page: alternating blocks. A data-heavy screen: a top-and-left shell. A portal: a hub. State why. Two match equally: state both, name the one you will write and why, and confirm before writing the file. None matches: ask which of the six, and do not invent a seventh. Write it as one self-contained HTML file with a small grayscale custom-property set, a system font, and no decoration. Then annotate, since the annotations carry what a grayscale block cannot: interaction behavior, content rules, conditional content, and what changes at each breakpoint.
...
The output is the observed system handed to `skills/Design System/` to compose into a specification and token file, together with what was approximate, what was inferred, and what needs a person to confirm. Extraction reports what is there, inconsistencies included. Did one role come back with two values? No: report the one value. Yes: report both and mark them unresolved. Do not pick one here. Does the request name which of the two stands? Yes: Phase 2 states that one. The request names neither: Phase 2 confirms with the requester before either value is used. Resolving a conflict is a design decision and does not happen inside this read.
...
- **A vague request answered with a questionnaire.** Twelve questions read as an intake form and stall the work. Phase 1's question on the four required items caps the ask at three, each with a recommended default; infer only what that question says to infer, and state it.
