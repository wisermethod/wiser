...
With recipe `retrieval: embedding`, a paraphrase question is **two recall calls with a choice between them**, the same shape as the wiki branch above, where you read an index and choose pages. The choice is yours, made as step 2 says.
...
2. **Choose.** Ask these questions in order. Each one states what a yes does and what a no does. A yes that says to choose as the chooser stops the later questions.
   ...
   Is this run a measurement of the set, an eval or a sitting whose record will score the recall? Yes: choose as the chooser below. That block is the measurement path. No: ask the next.

   Does `session_permission` name who permitted this session and the date? **This is the same check as the one at the top of this step, restated here only as the gate on the choice.** **No: stop. Do not fall through to the chooser**, because the chooser reads passage quotes and that is opening source material, which is exactly what the check at the top of this step forbids without permission. Say which step cannot run and get permission. **Corrected 2026-09-21 by adversarial review**, which found this branch sending a missing permission to a chooser that then reads the material the permission was for. Yes: choose as the chooser below.
