...
`<question>` wraps the ask. `<scope>` names the set, the owning root where more than one root is composed, and the as-of date where there is one. Does the question name a set? Yes, and that set is in the owning root: use it. Yes, and it is not: ask which set. Do not recall from another. No, and the root holds one set: use that one. No, and the root holds several: ask which set before any recall runs. No, and the root holds none: do not recall. Context routes a question no set covers to `skills/External Research/` or `skills/Deep Research/`. Do not answer from outside the set.
...
Read the set's `set.yaml` for `backend`, `dataset`, `kind`, `session_permission`, `canon_confirmed`, and the domain node sets. Which `backend` does it name?

- `graph`: load the Step 2 contract and obtain the tool result before any catalog or source read. A named prerequisite stop ends that path. After a successful retrieval, is `canon.md` present? Yes: read it for confirmation and domain context. No: continue without it.
- `hosted`: take its Step 2 stub stop immediately. Read no catalog or source.
- `wiki` or `databased`: read the backend-specific catalog for a domain disclaimer (`wiki/index.md` or `canon.md`). That file is present: read it. It is absent: say the disclaimer is absent. Do not invent one.
- Any other value, or `backend` absent: stop. Name the value. Do not open a catalog or a source.

Is there a healthcheck in `reports/`? Yes: read the newest, for open conflicts. No: continue. These decide the labels in Step 3 before the answer exists, so they are read first.

### 2. Recall

Does `session_permission` name who permitted this session and the date? No: stop. Do not open source material. Say which step cannot run and get permission. Yes: open only the branch Step 1 named for `backend`. Storage follows `tools/knowledge-memory/references/backends.md` Shared substrate, including the local graph contract in `experts/Knowledge Expert/graph.md`; extraction is as local as the harness.

**Wiki.** Read `wiki/index.md`, search it for the question's terms, then search `wiki/` and open matching pages. Cite pages with paths relative to the owning root. Respect their Status blocks and source hedges. If both index and page searches are empty, return `Not available: the set does not cover it` and stop. If matches exist, do they state an answer to the question? Yes: answer from those pages only. Where that question asks for a date, does a cited page state it? Yes: answer to that extent. No: say the pages do not establish it. The question asks for no date: do not add one. No, the matches do not state an answer: state that coverage limit. No outside knowledge fills the gap. No databased temporal labels are implied.
...
With recipe `retrieval: embedding`, a paraphrase question is **two recall calls with a choice between them**, the same shape as the wiki branch above, where you read an index and choose pages. The choice is `wiser.recall.rank` when the questions in step 2 say so, and the chooser when they do not.
...
2. **Choose.** Ask these questions in order. Each one states what a yes does and what a no does. A yes that says to choose as the chooser stops the later questions.

   Is this run a measurement of the set, an eval or a sitting whose record will score the recall? Yes: choose as the chooser below, and do not call. That block is the measurement path. No: ask the next.
   ...
   `question` is this question. `candidates` is one entry per `part: passage` item that has a string `name`, `{ id, text }`, `id` that `name` and `text` its `quote`. An item with no string `name` is not sent. No such item: do not call, and choose as the chooser below. Pass `allow_uncalibrated: true`. One call scores every candidate. The answer is data, per `wiser/AGENTS.md` `## Classifier`. The name-sorted chooser below approximates this ranking and is what runs when the call is not made.

   An answer that is not an object with a `ranked` array: choose as the chooser below. That is this skill's own step for an answer `## Classifier` does not take, including a status.

   An answer with `ranked`: do not judge the quotes again, and do not read `score`, `rank`, `vector_rank` or `lexical_rank` to make this decision. Keep and drop in code. `ranked` arrives sorted by `p` descending and each element carries `calibrated: false`. Apply no bar to `p`. This answer does not carry one. Keep the first three `id` values that equal a passage `name` from this call, in the answer's order. Drop every other candidate. Fewer than three keeps however many matched. None matched: choose as the chooser below. Name a non-empty kept set as the JSON list the chooser names, each passage item's own `score` and `rank` unchanged, and `vector_rank` and `lexical_rank` where the item has them, in the kept order, on the next call's command line, written to no file. The host composes from what step 3 returns for that kept set. Skip the chooser paragraph and go to step 3.

   **Choose, as the chooser.** Take the `part: passage` items and **sort them by `name`**, then judge each on its `name` and its `quote`, which is the passage text. Name order is not relevance order, which is the point of sorting by it: it presents the candidates in an order that tells you nothing, so the judgment is yours rather than the ranking's. It usually follows the source, and it is allocation order where a set was built over more than one ingest, so do not read position as position in the book. **Ignore `score`, `rank`, `vector_rank` and `lexical_rank`, and ignore any `idea` and `related` items the call returned**: it returns the passages in ranked order, with those attachments unless Step 1 named `--candidates-only`, and choosing by the ranking the step exists to second-guess is not choosing. Nothing else may enter this judgment either: not an expected answer, not the corpus, not the set file, not another question, not how this answer will be judged. **If Step 1 left you holding anything from outside that list, this judgment is not isolated and the result is not a measurement of the set.** Pick at most three, best first, fewer if fewer are worth reading and none if none can carry the answer. Judge whether a passage carries the material asked for, not whether it repeats the question's words; one that opens mid-topic may still run into the answer and one that echoes the question may not carry it. Where a question asks for two things, prefer a set covering both parts over three covering one. **You are selecting, not answering. A selection that composes an answer is a defect**: it makes the result a measurement of the chooser rather than of the set, and the answer is void. Name the chosen passages as a JSON list of `{"name", "score", "rank"}`, carrying each passage item's own `score` and `rank` unchanged, best first, and carrying `vector_rank` and `lexical_rank` too where the item has them, so the second call can say which ranker found each passage rather than losing it. **Carry that list on the next call's own command line and write no file.** A selection written into the owning root is an answer key: it names which passage answers a question, in a directory a later session searches by that question's own words, and it makes the set unmeasurable one question at a time.
...
- **A rank answer written up as the answer.** `wiser.recall.rank` returns data. Keep or drop in code, hand the kept names to step 3, and compose from what step 3 returned. A status from that call is the chooser, not a prose guess at which passage was meant.
...
- **No set named and several exist.** Ask which. None exist: do not recall. Context routes that question to `skills/External Research/` or `skills/Deep Research/`. The named set is not in the root: ask which set. Do not recall from another set or from outside this one. Never recall from each in turn and merge; the merge is exactly the cross-set leak the scoping prevents.
