...
`<question>` wraps the ask. `<scope>` names the set, the owning root where more than one root is composed, and the as-of date where there is one. A question that names a set in the owning root uses it. A named set that is not in the root is asked about, and is not recalled from another set. A question that names no set, where the root holds several, gets the question before any recall runs; where the root holds one, that one. Where the root holds none, do not recall. Context routes a question no set covers to `skills/External Research/` or `skills/Deep Research/`. Do not answer from outside the set.
...
Read the set's `set.yaml` for `backend`, `dataset`, `kind`, `session_permission`, `canon_confirmed`, and the domain node sets. For `graph`, load the Step 2 contract and obtain the tool result before any catalog or source read; a named prerequisite stop ends that path. After a successful retrieval, read `canon.md` if present for confirmation and domain context, and continue without it when it is absent. For `hosted`, take its Step 2 stub stop immediately; read no catalog or source. For `wiki` or `databased`, read the backend-specific catalog for a domain disclaimer (`wiki/index.md` or `canon.md`), and when that file is absent say so rather than inventing one. Any other `backend`, or none, stops the run: name the value and do not open a catalog or a source. Read the newest healthcheck in `reports/` if one exists, for open conflicts. These decide the labels in Step 3 before the answer exists, so they are read first.

### 2. Recall

Check session permission before opening source material. Without a `session_permission` that names who permitted this session and the date, stop, say which step cannot run, and get permission rather than opening source material. With it, open only the branch Step 1 named for `backend`. Storage follows `tools/knowledge-memory/references/backends.md` Shared substrate, including the local graph contract in `experts/Knowledge Expert/graph.md`; extraction is as local as the harness.
...
With recipe `retrieval: embedding`, a paraphrase question is **two recall calls with your own judgment between them**, the same shape as the wiki branch above, where you read an index and choose pages. The choice is yours. The chooser reads passage quotes, so a missing permission stops at the top of this step and does not fall through to this choice. **Corrected 2026-09-21 by adversarial review**, which found a branch sending a missing permission to a chooser that then reads the material the permission was for.
...
- **No set named and several exist.** Ask which. None exist: do not recall; Context routes that question to `skills/External Research/` or `skills/Deep Research/`. The named set is not in the root: ask which set, and do not recall from another set or from outside this one. Never recall from each in turn and merge; the merge is exactly the cross-set leak the scoping prevents.
