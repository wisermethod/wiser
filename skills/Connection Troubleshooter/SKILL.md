...
- **A `disconnect` `denied` carrying `orphaned_record_without_privilege` or `orphaned_record_without_provider` is about the record, not the caller's role.** The connection row outlived whatever declared it and carries metadata no manifest ever validated, so the gateway cannot establish what it may do or which provider it belongs to and refuses rather than guessing. **Do not suggest a different role and do not invent the missing value**; a role change cannot fix a row that names nothing. Report the field the answer names, and that clearing this record needs somebody who knows what it was.
