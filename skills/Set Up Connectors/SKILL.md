...
1. Where does this session's `gateway/server.js` resolve? Exactly one composed root whose `AGENTS.md` frontmatter declares `root: wiser`, and that root contains `gateway/server.js`: use that file. No such root, and exactly one loaded plugin directory contains `gateway/server.js`, including a Cowork or other plugin install: use that file. None, or more than one: stop and ask for the host or loaded plugin path that is missing. Never guess a home-directory path, use `memory/secrets/`, or run `git init`.
...
4. Call `list_connections` with `{}`. Follow one branch and stop:
   ...
   - Otherwise the key is set. `list_connections` hydrates ACTIVE grants this user id already has at the provider. Do not reconnect an ACTIVE row. Then:
     - They named a service and a module, and that pair is ACTIVE: say so.
     - They named a service and a module, and that pair is not ACTIVE: this skill is done. Connect Account is the next turn for that pair only.
     - They named a service and no module: ask for the module. Do not start Connect Account.
     ...
     - They named no service, and the list has rows but none is ACTIVE: this skill is done. Report that none are ACTIVE. Do not mention Connect Account. A vendor grant is a later turn, and only when they name the service.
