...
1. Resolve `gateway/server.js` for the loaded copy. Use the file in the one composed root whose `AGENTS.md` frontmatter declares `root: wiser` and that contains `gateway/server.js`. Where no such root exists, use the file in the one loaded plugin directory that contains `gateway/server.js`, including a Cowork or other plugin install. Where none exists, or more than one could, stop and ask for the host or loaded plugin path that is missing. Never guess a home-directory path, use `memory/secrets/`, or run `git init`.
