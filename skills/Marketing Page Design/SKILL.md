...
1. **Define the page's job.** Every page has one primary job; state it before designing. Name the single most important action a visitor should take, the information hierarchy that serves it, and the entry context. Record the one entry the brief names, one of ad click, search, referral, or direct; where it names more than one, ask which is the primary entry, and where it names none, ask. A vague brief such as "design a landing page", with no product, audience, or action, is answered by asking what the product is, who the audience is, and what the primary action is, not by guessing. Where the brief's material already holds a single candidate, offer that candidate as the recommended default; where it holds several, offer them and ask which; where it holds none, ask that question with no default.

2. **Set the visual direction.** Design from references, never from adjectives. Ask for or propose two or three real products or brands that fix the aesthetic; a direction like "Swiss editorial meets warm minimalism" is usable, "clean and modern" is not. When the user cannot name references, offer three from different aesthetic families and ask which resonates. The one they pick is the direction. Where they pick more than one, ask which one leads and let the others only sharpen it. Where none of the three resonates, ask them to name a reference. When `design` is bound, its brand rules and personality are the direction, and references only sharpen it.
...
5. **Design each section, top to bottom.** For each section: place content on the grid, apply the type hierarchy, apply color, set spacing, and check its relationship to the sections above and below. Run the Squint Test before moving to the next: blur the section in your mind's eye. The focal point is the element the beat exists to show: the headline and the primary action in the hero, the pain in Problem, the product's answer in Solution, the lead feature in Features, the proof in Social proof, the price in Pricing, the action in Call to action, the links in Footer. Where that focal point resolves first, move to the next section. Where it does not, change size, weight, or placement and run the test again. Where it still does not, ask which element should resolve first, and do not move on with a failed test. Content here is realistic placeholder, not Lorem ipsum: specific product detail, named example testimonials, concrete metrics, so the design can be judged at fidelity. Mark it as placeholder; before the page is published it is replaced with real copy, and any testimonial or fact about a real person then carries its source and register (`standards/conventions.md`).

6. **Verify the hero.** The hero is the page's first impression and sets the tone for everything below. It carries a headline (the core value in six to ten words), one or two supporting sentences, one primary action, and a visual anchor. Choose the hero pattern that fits the product (Reference: Hero patterns). Where two When lines are true, ask, name each pattern, and build the one they pick. Where none is true, ask. Then run the Squint Test on the above-the-fold area: the headline, the action, and the general purpose must all read at a blur. Where that blur does not read all three, change size, weight, or placement and run it again. Where it still fails, ask which of the headline, the action, and the purpose is not reading, fix that one, and run the test again. Where it still fails, stop on the hero and ask, and do not continue on a failed Squint Test.
...
8. **Define responsive adaptation.** Per section, apply the breakpoint table. This is design, not a shrunk desktop. A hero image may drop on mobile, a feature row may stack, the primary action may become full width. Make a change only when it keeps the primary action visible and leaves a statement of what the product is; otherwise keep the element, including when you cannot tell.
...
The identical card grid is a prohibited default (Prohibited defaults, above). Use the layout the requester or the bound `design` names. Otherwise use Asymmetric grid when the brief marks one feature as primary and the others as secondary, Bento grid when the brief supplies mixed media at different sizes, and Progressive disclosure when the brief asks for a few features to stay visible and the rest to sit behind an expandable region. Where two of those hold and nobody named one, ask which, name what each would make visible, and build the one they pick. Where none of them hold, use Alternating layout. The four:
