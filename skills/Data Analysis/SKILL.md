...
- **question**, optional: what the caller wants to know. Absent, the request is an open analysis: the profile, statistics for the quantity columns the test in Step 2 selects, and a breakdown by a text column. The profile cannot show how many groups that column has. Step 2 runs the aggregate and reads `groupCount`.
...
| Join two files on a shared key | Profile each side, then `join` on the key; never match rows by reading them into the conversation. Did the requester name `inner` or `left`? `inner`: use `inner`. `left`: use `left`. Neither, or they do not say: `inner` is the default. `inner` silently drops every unmatched row. `left` keeps them visible. Read `leftRows`, `rightRows` and `matchedRows` from the result, and state any shortfall in the analysis, because a total computed over a partial join is wrong by exactly the rows nobody saw |
| A percentage of a total, a difference, or a rate between two returned figures | `compute` reads one JSON object, so where the two figures sit in two results, save each result and write one file that holds both objects under two keys, copied whole and unaltered; then run `compute` on it, naming the two fields by their dotted paths, and report what it returns, its `error` included, and nothing computed by hand |
...
Which numeric columns are quantities? For each column the profile types as a number, do its name and its sample values show a quantity you would sum or average? Yes: it is a column worth describing. Name it to `describe`. No, and they show an identifier, a year, a postal code, or a 0-or-1 flag: do not describe it. Those means are noise. You cannot tell from the name and the samples: ask before `describe`. Do not describe that column until they answer. No answer: leave it undescribed and name it. **Naming columns empties `skippedColumns`**, which then reports only columns you named and it could not use, never the ones you did not name. So when the run narrowed the set, the columns present but undescribed are named in the analysis from the profile rather than from that field, or a nine-column file reports truthfully that nothing was skipped while seven columns went undescribed.
...
- A non-numeric column asked for a numeric function: does a count of that column answer what was asked? Yes: re-run it as a count, and say so. No: drop it, and say so. Do not re-run a numeric function under a softer argument to make the asked-for figure appear. You cannot tell whether a count answers it: drop it, say so, and ask. Do not invent the count.
...
What the analyst adds is not figures. It is which of them matter, what is unusual against the others, what the file is missing, and what it cannot answer. Comparing and ranking figures that came back is reading the results. Producing a figure that appears in no result is arithmetic, whatever its size: a percentage, a difference, a rate, a total of two totals, a rounded restatement. The test is one question, asked of every number before it is written: which field is this? A field you can point at: the figure may be written. No field: do not write the figure.
...
Then the gate: hand the analysis, with the question it answers and where it is going (unnamed, the response itself), to `experts/Research Expert/` for its Job 3, in a second context. It judges whether each figure was measured, naming the result field, or read, and whether a stop on an operation the tool lacks was the right result or a `compute` the run did not ask for. What did it return? Rely: the analysis reaches its consumer. Rely with the weak points named and labeled: a deliverable or a decision may take it with those labels in the delivery; a memory file does not, per that expert's rule on the consumer, and the analysis goes back to close them. The requester explicitly declines: it reaches its consumer, and the delivery names the decline. Neither: do not deliver it as finished.
...
- **The request that has not been asked yet.** "Analyze this" over a file of forty columns is the case where the quantity test cannot settle the columns from names and samples. A question naming a column that is not in the profile, or a grouping that could be more than one text column, is the same stop. Ask which columns or which question before `describe`, `aggregate`, `join`, `chart`, or `compute`. `parse` has already run. It is the profile Step 1 requires, and it answers most of the ask. Do not guess the analysis. A guessed analysis is expensive and looks finished.
