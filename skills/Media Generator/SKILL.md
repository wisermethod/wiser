...
1. **Place the request.** Anything the Context hands to a tool goes there and this skill stops. Decide which of three it is. An image that does not exist yet, and not a video and not a background removal, is an image to generate. A video or a clip, including a still to animate, is a video to generate. A photograph whose background comes off, with no new scene generated, is a background removal. Where none of the three fits, or two of them do, ask before any billed call, and do not bill without an answer. Then settle two facts before anything is billed. The destination: there is no default save location, so ask for the directory and the filename, and keep intermediate frames in a work directory per `standards/conventions.md`. Do not bill without a directory and filename. And the purpose, because purpose picks the medium in Step 2. Use a purpose the request states. Where it states none and the surrounding work states one, the deck, page, post, or file this image is for, use that and say so. Where neither states one and the user is present, ask rather than assume. Where neither states one and nobody is present to answer, apply the defaults in Step 2 and say in the delivery that you did. A background removal writes no prompt, so it skips Step 2 and runs Steps 3 to 5 in the background-removal category.
...
2. **Finish the prompt.** Every request passes this assessment, including one that arrives looking complete. Where the user supplies a still to animate, that image has already fixed the medium and the look, so the assessment runs on what changes, the motion, and the medium row below is already answered. Then, before any billed call, `experts/Creative Director/` direction, in this context: hand it the purpose, the audience, what matters most and the medium as `<brief>`, and take back the references and the register the prompt carries, or the requester declines that direction and the delivery says so; the table above fills the medium, the direction refines register and references, and a medium the user stated beats both.
   ...
   Medium comes from purpose, and a medium the user stated is never overridden. One matching row is that row. Where two rows match, ask which purpose governs, and do not bill without an answer. Where no row matches, use Photorealistic photograph, the Nothing-to-go-on row, and say so.
...
3. **Choose the model and the frame.** Call `replicate.models.list_collections` with `{}` (`confirmation: none`) and choose for the category you need, image, video, or background removal. Where the user named a model and the list or its public page carries it, use that one. Where the list names one whose category matches and whose public schema accepts the inputs this job needs, use it. Where several pass and the user named none, select among them and say why: what this job needs that the choice serves better, an input, a schema constraint, or a difference the brief settles, such as price, speed or visual suitability. Where nothing the brief or the schemas state tells them apart, ask, and do not bill without an answer. Where the list names none that pass, search the platform's public collections and apply the same test. Where none still pass, stop and tell the user, and do not invent a model. Say in the delivery which model you chose and why; the connector infers no model or version.
   ...
   Frame last, where a frame is being composed. Where `<style_requirements>` states no aspect ratio and no pixel dimensions, do not pick a ratio. Where the user stated a ratio the schema lists, use that ratio and do not replace it with a closer one. Where they stated pixels, or a ratio the schema does not list, pick the closest ratio the schema lists, tell the user the pixel size that ratio actually delivers, and send exact dimensions to `tools/image/` `edit` afterward rather than hunting for a model that outputs them natively. Where two listed ratios are equally close, ask, and do not send a ratio or bill without an answer.

4. **Run the generation.** Say what the run will cost, in shape if not to the cent, and say how many billed calls the request requires, before the first call. One image, one photograph, or one still to animate is one call. Video is one call per animation segment, a clip longer than the chosen model's single run taking one segment per run, joined later by `tools/video-edit/`, never one longer prompt, and video from text alone adds one call first for the still, so a text-only clip in three segments is four calls. Several photographs are one call each. Where you cannot tell the count, ask, and do not start. Call `replicate.models.create_prediction` with `{ version, input }`. Its gateway confirmation is `always`: on `needs_confirmation`, wait for the person to say yes, then repeat the action with `confirm: true`. Every run stops, not only the first of a session. Spend disclosure accompanies that gateway confirmation.
   ...
   A still handed to an image-to-video model has to be reachable by the platform: send it in an inline form the chosen model's documented input contract supports, or at an address the platform can fetch. The connector states no inline size ceiling; do not invent one. A larger local file with no address does not go as it is, so say that and put the two ways forward, a smaller rendition made by `tools/image/` `edit` or an address the platform can reach, and do the one they choose. Where they choose text-to-video instead, say that the still was the point and that this is a fallback, and only then generate from text. Where they do not answer, do not send the file as it is, do not fall back, and do not bill.
...
5. **Remove a background.** This is Step 4's call with a photograph where the prompt would be: the same connector, the same spend disclosure, the same schema read from Step 3, and the same documented input reachability on the image going in. Several photographs are several runs, each billed, which is worth saying before the first one. The direction Step 2 takes before a billed call is taken here too, in this context, on the cutout's purpose and where it will sit, before the call is made, or the requester declines that direction and the delivery says so.
   ...
   Two schema fields are worth looking for by name. A model offering a human-segmentation variant gets it whenever the subject is a person, because a general model cuts a person badly at the shoulders and the hair. Where the subject is a person and the schema offers none, use the general model and say so. Where you cannot tell whether the subject is a person, ask, and do not guess. And alpha matting, where the model offers it, is on: it keeps hair, fur, and soft edges from turning into a hard sawtooth. Its thresholds are the model's own fields, so read them there rather than carrying numbers between models. Where the schema does not offer alpha matting, do not invent thresholds.
