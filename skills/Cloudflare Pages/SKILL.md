...
type: skill
category: web
description: List and get a Cloudflare Pages project, list its deployments, create a project, add a domain, and deploy the kit site/dist/ payload, with confirmation on every deploy
version: 0.3.0
---
...
Use when the job is a Cloudflare Pages project or taking a kit site live on Pages: list or get the project, list its deployments, create a project, add a domain, or deploy the kit payload.
...
`account_id` is required for every gateway call and comes from the requester. Ask if it is missing; the finding-ids route in `connectors/cloudflare/auth.md` is not an action this skill uses. Get, deployment-list, add-domain, and deploy also require `project_name`; never select the first project implicitly. Deploy's `dir` is the absolute path of that kit's `site/dist/`. No memory key is requested.
...
**1. Settle the job and scope.** Choose project-list, project-get, deployment-list, create-project, add-domain, or deploy from `<request>`. Missing or ambiguous account, project, domain, or site: ask before proceeding. Apply Context's hand-offs and the payload refusal before selecting an action.
...
- **A domain mistaken for DNS.** `add_domain` does not create the CNAME. The hostname stays pending until Zone Publisher publishes that record.
