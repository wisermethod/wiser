...
**2. Fix what voice means for that type.** The owning root's AGENTS.md governs what its voice means; the table above adds the evidence filter and the confirming authority. If the voice the requester actually wants belongs to a different root, the file to build is that root's, so return to step 1. For a department, read the parent organization's bound voice before deriving. Unreachable, note the comparison as thin, and do not claim the register differs. Whether the register differs is stated in the file at step 9, once the traits exist.
...
**4. Gather evidence, weighted by that answer.** Ask for at least three pieces from different contexts, of the kind the table names, taken first from the material that addresses the readers step 3 named. Judge each piece by whose voice it carries. For the personal row, what the person wrote themselves counts, including model-assisted writing they directed, edited, and stand behind under their own name. Record that origin on the evidence row. A ghostwritten piece they did not edit encodes the writer who was hired. A model draft they did not review does not count. For the org and client rows, material published or approved under the subject's name counts even when a third party drafted it, model-assisted or not; material about the subject in someone else's voice, a filled-in template, and a forwarded document never do. Anything doubtful, ask which pieces the subject stands behind under their own name and set the rest aside. A piece that survived and addresses a reader step 3 named counts toward the three, ahead of any piece that does not address those readers. One that survived but sits in a register the outputs do not use does not weight register-specific traits.
...
Do not prop a directed decision on derived evidence. If a human set the register, it stands on their instruction and needs no corroboration. Corroboration that is offered anyway must be correct, counted from the material on disk rather than asserted from impression, because a directed decision resting on a bad measurement looks unsafe the moment someone checks the measurement. Where the count can be made from the material on disk, record it. Where it cannot, leave the decision standing on the instruction alone, and do not assert the measurement.
...
**9. Draft the file.** Keep the headings the root's own stub carries, whose speaker follows the declared type, and keep the standing sentence above them that routes prose written from this file, which is not a prompt line to replace; replace every prompt line with content. Draft in the session or the owning root's `work/` directory; the bound path is written only in step 10. A section with no evidence behind it says what is not yet known and what would settle it; it never guesses. Describe patterns rather than pasting passages, because a pasted paragraph is reproduced verbatim in later work. General craft belongs to whoever writes the content; this file holds only what is specific to this voice. The governing register decision and its confirmation each stand as their own statement in the file, because they are the voice key's load-bearing claims. For a department whose parent voice was not readable at step 2, the file carries the thin note and does not claim a difference. Where it was readable, the file states each derived trait or stated rule that differs, with the evidence, and where none differs it says the register does not differ. Where personal evidence included model-assisted samples, the file says the corpus included them, so a later rebuild can re-weight.
...
**10. Verify, confirm, then write the bound path.** Write the sample in the derived voice, kept in the session or the owning root's `work/` directory, on material this voice would govern, and never on a passage used in derivation. The sample takes the output type the routing table names as most of the work. Where it names none, ask which type the sample should take, and do not pick the first row. Where that type's row names a length standard of at least 150 words, write one sample that meets it. Where the standard is under 150 words, a character cap included, write samples that each meet that standard, enough of them that together they come to at least 150 words, and do not write one sample past the standard to reach 150 words. Where no standard is named, write at least 150 words. Hand that rewrite, wrapped in `<draft>`, to `experts/Ghost Writer/` in a second context that did not derive the voice, as the intended reader of the output type used for the sample; work its findings before the confirming authority sees the rewrite. Show it to the confirming authority from the table, with the routing table and the decision list, and ask two things: whether it sounds like the subject, or for an industry register whether it reads as the field writes; and whether each row routes the right register to the right output.

A vague answer: ask which of tone, vocabulary, structure, or stance is wrong, then fix each named section and rewrite again. Where they name none of the four, or they do not answer, ask once more, and if they still name none, stop before another rewrite and do not revise the whole file in its place. Every rejected attempt is archived per `standards/conventions.md` with one line saying what it got wrong, because an archive of attempts with no diagnosis is a pile of drafts. A rejection that arrives with a rule is an overrule: step 6.
