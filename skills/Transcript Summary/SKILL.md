...
- **Coverage.** Every stretch the coverage question in step 6 names has a home in the summary, or is named as one the summary leaves out and why. A key point or a nuance lost on the way out is a failure, not a tighter summary.
...
Can the substance of what was said be recovered from the text as it stands, as points, decisions, and questions? Yes: summarize, and each defect found here bounds what the summary may claim. No: do not summarize. Say what is unreadable and what would fix it.
...
- **Decisions.** Did the recording state that something was decided, or state an outcome the room would act on? Yes, and it states the reasoning: record both. Yes, and it states none: record the decision and mark it decided without stated reasoning. No, it was raised and left unresolved: it is an open question, not a decision. You cannot tell: record it as an open question and say the recording did not state a decision.
- **Action items.** Does the recording name an owner? Yes: record that owner. No: record the item with the owner missing; do not assign one. Does it name a timing? Yes: record it. No: name the timing as not given.
...
**5. Write the analysis from the finished record.** For each item in the record, would a reader who was not in the room miss a decision, something they owe, an open question, or what the room was like, if this item stayed out of the analysis? Yes: it leads, and the cost named is which of those four they would miss. No: it stays in the record and does not lead. Then the dynamics from step 3, the verification results from step 4, and the references the recording mentioned, carrying the citations step 4 found. The analysis is written last and read first.

**6. Check coverage, assemble, then gate.** Walk the transcript start to end against the assembled summary. For each stretch, which is it? A decision, an action item, an open question, a specific point (a figure someone gave, a constraint someone named, or an objection someone raised), or a quote carrying a position, a commitment, or an insight: it has a home in the summary, unless the request narrowed the summary to a slice that leaves it out, in which case the one line below names what the recording held outside that slice. None of those: it is minor. Name it, or name the group of such stretches, and say it is minor. Do not drop either kind unnamed. Which line of the summary traces to neither a transcript passage nor a step 4 source: it comes out.
...
- **Transcription artifact promoted to fact.** A garbled name or a misheard figure repeated as though the recording said it cleanly. Does a proper noun or a number read as mangled? No: leave it. Yes, and a decision, an action item, or a checkable claim rests on it: flag it where it appears, and step 4 settles it. Yes, and nothing rests on it: flag it where it appears, and do not repeat it as clean.
