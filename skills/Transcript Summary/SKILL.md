...
A transcript too degraded to recover the substance from, as points, decisions, and questions, is reported as that rather than summarized: say what is unreadable and what would fix it.
...
- **Decisions.** Where the recording states that something was decided, or states an outcome the room would act on, record it, with the reasoning where the recording states one, and marked as decided without stated reasoning where it does not. Where it was raised and left unresolved, it is an open question, not a decision. Where you cannot tell, record it as an open question and say the recording did not state a decision.
- **Action items**, each with owner and timing where the recording gives them. One that nobody claimed is still an action item: record it with the owner missing rather than assigning one. Where the recording names no timing, name the timing as not given.
...
**5. Write the analysis from the finished record.** Critical points first. An item leads when a reader who was not in the room would miss a decision, something they owe, an open question, or what the room was like if it stayed out of the analysis, and the cost named is which of those four they would miss. Anything else stays in the record and does not lead. Then the dynamics from step 3, the verification results from step 4, and the references the recording mentioned, carrying the citations step 4 found. The analysis is written last and read first.

**6. Check coverage, assemble, then gate.** Walk the transcript start to end against the assembled summary. A decision, an action item, an open question, a specific point (a figure someone gave, a constraint someone named, or an objection someone raised), or a quote carrying a position, a commitment, or an insight has a home in the summary, unless the request narrowed the summary to a slice that leaves it out, in which case the one line below names what the recording held outside that slice. A stretch that is none of those is minor: name it, or name the group of such stretches, and say it is minor. Do not drop either kind unnamed. A line of the summary that traces to neither a transcript passage nor a step 4 source comes out.
...
- **Transcription artifact promoted to fact.** A garbled name or a misheard figure repeated as though the recording said it cleanly. A proper noun or a number that reads as mangled is flagged where it appears. Where a decision, an action item, or a checkable claim rests on it, step 4 settles it. Where nothing rests on it, do not repeat it as clean.
