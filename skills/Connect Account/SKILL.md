...
4. Call the gateway tool `start_connect` with `{ service, module }` and handle its result:
   ...
   - `vendor_error`: report `http_status` and `endpoint` only, never the body. The first one, not called stuck, stops there. Stuck, or the same `vendor_error` already reported this turn, routes to Connection Troubleshooter.
   - Any other result: report the `status` received and use Connection Troubleshooter.
5. After the person says they approved or wrote the file, call `connect_status` with the same `{ service, module }`. `status: connected` completes the grant when no provider status is present or it is `ACTIVE`; a `connected` result with any other provider status is named and is not a completed grant. `INITIATED` means they have not finished: wait and ask, without polling in a tight loop. `needs_connect` with `provider_status` `EXPIRED`, `FAILED`, or `INACTIVE` is still not connected; name it. For another stop, use the status handling above or Connection Troubleshooter. Do not write `Last connected` into this plugin, which is read-only in use. **And there is no date to record.** Since 2026-09-20 that section holds `Yes.` or `Not yet.` and nothing else: no date, no account, no machine, no harness, no vendor response. During authoring-Playbook work, a first successful connect turns a `Not yet.` into a `Yes.` and changes nothing else; everything the run observed is build evidence and goes where that Playbook keeps evidence.
