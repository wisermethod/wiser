...
4. Call the gateway tool `start_connect` with `{ service, module }` and handle its result:
   ...
   - `vendor_error`: report `http_status` and `endpoint` only, never the body. Where the person has hit this error once and has not called it stuck, report those two fields and stop. Where they say they are stuck, or the same `vendor_error` has already been reported this turn, route it to Connection Troubleshooter. Do not ask for a key, and do not work around the error.
   - Any other result: report the `status` received. Do not ask for a key, do not invent a connector, and do not call `start_connect` again. Use Connection Troubleshooter.
5. After the person says they approved or wrote the file, call `connect_status` with the same `{ service, module }`. Until they have said so, wait, and do not call `connect_status` or `start_connect` again. `status: connected` completes the grant when no provider status is present or the provider status is `ACTIVE`. A `connected` result whose provider status is present and is not `ACTIVE` is named and is not a completed grant. `INITIATED` means they have not finished: wait and ask, without polling in a tight loop. `needs_connect` with `provider_status` `EXPIRED`, `FAILED`, or `INACTIVE` is still not connected; name it. A result step 4 already names is handled by that result's step. For another stop, use Connection Troubleshooter. Do not call `start_connect` again in this turn on any of these results, do not invent a connector, and do not take a key in chat. Do not write `Last connected` into this plugin, which is read-only in use. **And there is no date to record.** Since 2026-09-20 that section holds `Yes.` or `Not yet.` and nothing else: no date, no account, no machine, no harness, no vendor response. During authoring-Playbook work, when this call completed the grant and that section still says `Not yet.`, change `Not yet.` to `Yes.` and change nothing else; everything the run observed is build evidence and goes where that Playbook keeps evidence. Where any of those is no, or you cannot tell whether this turn is authoring-Playbook work, write nothing in this plugin.
