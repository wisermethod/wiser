...
2. **States.** Decide which of criterion 1's states apply to this component, then design each explicitly: default always; hover, focus and active when it can be operated; disabled when it can be made unavailable; loading when it waits on something; error when an input or an operation can fail; empty when it is a container that can hold nothing; success when it completes an action. A listed state with no moment on this component is not designed, and is named as not applicable. Where you cannot tell whether a state applies, ask, and do not omit a state that has a moment. Hover is scoped to `@media (hover: hover)` so a touch device does not hold a hover state after a tap. `interaction-design.md` carries the visual treatment for each: the eight states of an interactive element in one table, and the container's empty state in its own section. `motion-design.md` carries the timing of the transitions between them. Designing every applicable state here, before styling, is the discipline this skill exists to enforce.
...
5. **Visual design.** Where quality is won. A dense data application or an unusual shell is taken to `experts/Creative Director/` for spatial, density, and type-scale judgment before the hierarchy below; where you cannot tell, ask, and do not invent a shell. Set hierarchy so the primary element carries the most weight, across two or three of size, weight, and color. Use the two or three the user or the tokens name. Where they name one, keep it and add size, or weight if they already named size. Where they name none, use size and weight, and add color when the component has a call to action or an active state. One dimension is not hierarchy. Color, when it is one of them, is used semantically, primary sparingly for calls to action and active states. Spacing groups the related and separates the distinct. Typography maps to the type scale, and weight carries emphasis.
...
7. **Responsive behavior.** Define how the component adapts: reflow, resize, hide, or change of interaction. Use what the user named. Do not hide the only path to a critical action: reflow it, or change the interaction, and say so, and where you cannot tell whether a hide is critical, do not hide and ask. Where the user named none, use the Adaptation Patterns row in `responsive-design.md` for this element, including a hide that row names, and check that table before inventing a treatment. Where there is no row, reflow. Write the rules mobile-first with `min-width` queries. `responsive-design.md` carries breakpoints, input-method detection, and per-element adaptation.

8. **Output.** Emit one self-contained unit: a `<style>` block carrying the tokens, the component styles for every state designed in step 2, and the responsive rules. That block is the default when the caller did not ask for Tailwind. Where they did, put utility classes on the elements instead of a `<style>` block, and do not emit both. Where the caller asked for a verdict, hand this unit as `<design_artifact>`, the resolved tokens as `<design_system>`, and step 1's purpose with the audience and what matters most as `<brief>`, to `experts/Creative Director/` in a second context, and work the findings before delivering; otherwise the component ships on the checks in Success.
...
- **Navigation.** A current-page indicator that is more than text color, a background or a rule. A mobile collapse follows Step 7: a drawer or a bottom tab bar when the user named one, hamburger plus drawer from the Adaptation Patterns row when they named neither, and a question when they named both, with neither built until they answer. Keyboard-navigable, with no hover-only dropdowns; roving-tabindex detail is in `interaction-design.md`.
...
- **Variant explosion.** A request for every size by emphasis by color by layout is a matrix of thin components. Do not design all of them thinly. Ask which three or four are needed, design the ones they name, fully, and where they name more than four, say that more variants means each is thinner, recommend the three or four you would design, and design what they confirm. Where they do not answer, do not design the matrix. The base still follows Step 6.
