...
4. **Create or adopt.** On an adopt, read the existing constitution first and carry its standing constraints forward into the replacement verbatim; they are decided inputs, not draft text. Name anything the adopt would drop before dropping it, per `wiser/AGENTS.md` Irreversibles. **A paragraph the template already states in a section of its own is not a constraint to carry, and carrying it duplicates the constitution rather than preserving a decision.** Read each one against the template before transplanting it: where the template says the same thing, cite the template's heading and name the paragraph as not carried, so a person sees the drop and can reverse it; otherwise carry it forward verbatim. Where the adopt would change a constraint's wording, name the change, keep the original verbatim in the tree, and let a person decide. Do not write the changed wording in this run. Four domain plugins shipped with the composition rule restated under `## Standing constraint` because every placeholder had filed it there and this step carried it faithfully. **Settle every directory already under the destination before writing.** Where the person keeps no roster, or the roster has no row for a directory, stop for a person to give it one, and write nothing until they do. One the roster marks non-shipping is kept out of what publishes; one whose row says it ships stays in the tree; one the roster records no disposition for stops for a person to give it one. Do not guess.
...
- **The request is ambiguous.** The destination is missing, or the plugin class is unclear: ask before proceeding, and write nothing while asking. A brief that asks for a second base, or for a change to `wiser`, is a refusal under Context, not a question about class. **Create or adopt is not on that list**, because step 2 reads it off the destination; the one case that does stop is a populated, undeclared tree the brief never asked to adopt, and that stop is a scope decision rather than a question about wording.
