...
4. **Create or adopt.** On an adopt, read the existing constitution first and carry its standing constraints forward into the replacement verbatim; they are decided inputs, not draft text. Name anything the adopt would drop before dropping it, per `wiser/AGENTS.md` Irreversibles. **Settle every directory already under the destination before writing.** One the operator's repository roster marks non-shipping is kept out of what publishes; one the roster records no disposition for **stops for a person to give it one**.
