...
4. **Create or adopt.** On an adopt, read the existing constitution first and carry its standing constraints forward into the replacement verbatim; they are decided inputs, not draft text. Name anything the adopt would drop before dropping it, per `wiser/AGENTS.md` Irreversibles. **A paragraph the template already states in a section of its own is not a constraint to carry, and carrying it duplicates the constitution rather than preserving a decision.** Read each one against the template before transplanting it. Does the template already state this paragraph in a section of its own? Yes: do not carry it. Cite the template's heading and name the paragraph as not carried, so a person sees the drop and can reverse it. No: carry it forward verbatim. Would the adopt change its wording? Name the change, keep the original verbatim in the tree, and let a person decide. Do not write the changed wording in this run. Four domain plugins shipped with the composition rule restated under `## Standing constraint` because every placeholder had filed it there and this step carried it faithfully. **Settle every directory already under the destination before writing.** For each directory already there, does a roster the person keeps record its disposition? Kept no roster, or the roster has no row for it: **stop for a person to give it one**, and write nothing until they do. A row that records one: Non-shipping: keep it out of what publishes. A disposition that it ships: leave it in the tree. A row with no disposition: **stop for a person to give it one**. Do not guess.

**What publishes has two doors and they do not exclude the same files.** A harness pointed at a **git source** takes the commit, so an ignore rule keeps a directory out. A harness pointed at a **directory source** copies the working tree, where an ignore rule means nothing at all. Kept out of what publishes therefore means kept out of the tree, not merely out of the commit: a non-shipping directory that must stay on disk stays outside the destination, and one that cannot move is reported to a person as shipping, because saying it is ignored would be true of one door and false of the other. A large research or source directory inherited with a placeholder is the ordinary case, and a disposition guessed either publishes material that was never meant to ship or drops material nobody agreed to lose.
...
**On an adopt the archive lands outside the destination, and that overrides the convention's placement.** `standards/conventions.md` puts a `zArchive/` beside the file, which is inside a tree that is about to publish; by both doors above it would ship. Worse, the file being archived is a placeholder constitution, and a placeholder's constitution ordinarily cites the build ledger that authorizes its stand-up, which C4 scores misfiled in a shipped file. So the adopt would publish the one file guaranteed to break the clause it just scored present. Write the archive outside the destination, say in the report exactly where it went, and give it a `## Handover` line. Where the person has named an archive home, it goes there; where they have not, it goes beside the destination rather than inside it, which needs no question. Stop only where nothing outside the destination can be written at all.
...
5. **Produce the tree** from `system/templates/Plugin Root Template/`: the constitution with its `root:` id, plugin class, write mode and composition declaration, and the family directories, each with its index. Which families does the brief say ship? It names one or more: produce those. It says nothing: produce all four the template ships. Silence is not an instruction to delete what the template provides. It names a family to omit: produce the others. The constitution's family table carries one row per directory produced and none for a family this tree does not ship, which `{{FAMILIES}}` in the template is for. Do not ask which families exist. Then place the license that the declared plugin class entails, which is C8's check and not a second declaration of class.
...
**One line is owed wherever this root was described before it existed, and it is the one most easily missed.** Producing the tree makes every description of it as unbuilt, reserved or unscoped false, wherever those live: a parent constitution, a roster, a build ledger, a README in another tree. This skill opens one root and cannot reach them, which is why it writes the line rather than the repair. Does the roster, the brief, `<context>`, or anything the operator handed this session describe this root as unbuilt, reserved, or unscoped? Yes: name the line `descriptions of this root`, owed to whoever keeps them. No: write no such line. A create usually has none. Do not search a tree this skill does not open.
...
7. **Score the produced tree** against `standards/plugin-root.md`, clause by clause, and report every clause with the value it took: present, absent, misfiled or N/A. A clause you cannot decide is reported undecided; it is never reported present.

**A value measured by a search is reported with the search that measured it**, and three things make that search worth believing. It **states its exact invocation**, so a reader can run it again. It runs with **no exclusions** over the whole population the clause names, because an exclusion written by the person being checked decides the result before the check does. And it carries a **control that fires**: a string the population certainly contains, counted by the same command in the same run, proving the search can report a hit at all. A search whose control returns zero measured nothing, whatever its own count was, and its clause is reported undecided rather than present. **Then quote the produced `README.md`'s claims about what this repository holds, verbatim, beside the score.** C10 reaches behaviour claims, so a description of skills, experts or tools the tree does not carry is caught by no clause at all, and a tree can reach a clean score while its README describes a bench it holds none of. Quoting is not scoring: the clause values stand as measured, and the person reading decides whether the description is true.
...
- **The request is ambiguous.** Is the destination missing? Yes: ask before proceeding, and write nothing while asking. Is the plugin class unclear? Yes: ask before proceeding, and write nothing while asking. A brief that asks for a second base, or for a change to `wiser`, is a refusal under Context, not a question about class. **Create or adopt is not asked either**, because step 2 reads it off the destination; the one case that does stop is a populated, undeclared tree the brief never asked to adopt, and that stop is a scope decision rather than a question about wording.
...
- **The adopt carries something that was never a constraint.** A placeholder may have filed an ordinary composition or layout statement under its `## Standing constraint` heading. Carried verbatim into a template that already states it, it becomes a second, drifting copy under a heading it does not belong to, and the check below reads as satisfied because the text is present. Compare each one against the template first.
...
- On an adopt, every standing constraint the source declared is present in the replacement, or named as changed for a person to decide, **and no paragraph the template already states in its own section was carried into the constraint slot as well**.
