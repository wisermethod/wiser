...
An ambiguous case, an eval that fails with no source change, or a change that fits two rows: name the ambiguity and hand the judgment to `experts/Knowledge Expert/` rather than picking a row, whatever row the requester names. Several paths may run in one sitting, in the order incremental, review, apply, check. A rebuild or an upgrade among them is handed to `experts/Knowledge Expert/` rather than run with the others.
...
State the reason and source/chunk counts before rebuilding. Wiki recompiles from immutable `corpus/`, keeping Status blocks and archive pages, then runs the coverage pass in `tools/knowledge-memory/references/backends.md` Organizing pass so a load-bearing located idea cannot disappear because the old theme list was short, an idea whose importance is uncertain becoming a Candidate Idea with no page on that uncertainty, then checks the index and runs lint. A recompile is not permission to erase disputes.

Databased: check extraction reuse before dropping memory. An entry is reusable only when dataset, source hash, chunk hash and pack hash match. Report reused and new extraction counts. With the rebuild authorized, run `forget --memory-only --confirm`, chunk, extract only entries needing it, ingest every extraction, `promote --replay`, `review-pass`, then `healthcheck --eval`. Have a human read the composed answers. Compare with the last pack filled per `tools/knowledge-memory/references/backends.md` Databased eval; a template-floor self-hit is not that baseline. Report a regression, a row the last passing run passed that this run fails, or a wrong answer the human read names that the last run had right, instead of weakening a correct eval. Where no last passing run exists, report the comparison that could be made and do not call the missing baseline a regression.
...
- **The request names a set and no path.** Apply the table; if it still fits two rows, or an eval fails with no source change, hand it to `experts/Knowledge Expert/` rather than asking, whatever row the requester names. Do not run a rebuild because it covers every case.
