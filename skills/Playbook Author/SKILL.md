...
2. Is Status Paused? No: go to the next step. Yes: read the recorded blocker. Does the disk show it already resolved? Yes: record what shows it, and continue. No: does the Playbook name an action that removes it, one this session is authorised to take? Yes: run step 3's staleness check first. It finds the plan stale: follow step 3, and do not take the action yet. The plan still holds: take it, confirm on disk that the blocker is gone, record what you did, and continue. No, or the action ran and the disk still shows the blocker: escalate it, leave Status Paused, and do not resume past it.
