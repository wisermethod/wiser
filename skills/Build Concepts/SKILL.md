...
**3. Test what survives being obvious.** Four tests, run on each candidate: does it cut against what the field currently repeats, is it concrete enough to be shown wrong, would someone doing this work learn from it, and does it join ideas that were not joined before. In direction mode, does the candidate fail the falsifiability test, concrete enough to be shown wrong? Yes: it ranks low. Say so plainly, and do not carry it. Otherwise, how many of the four does it pass? One or more: carry it forward. None: it ranks low. Say so plainly, and do not carry it. Do not drop a candidate that passed one test, the falsifiability test included, for being only middling.

Did every candidate rank low? No: continue to step 4 with what you carried. Yes: do one more pass, looking for the insight in the synthesis rather than in any statement. That pass still ranks every candidate low: deliver that as the result: the material has nothing to teach. Stop the run there; it is a result, not a failed extraction. Do not accept that finding before the extra pass.

**4. Choose the one insight the rest serves.** Test each survivor: does it contain the others, can the others stand as its evidence, is it large enough to anchor a whole piece, is it specific enough to act on. Take the first match. Did one survivor contain the others, with the others able to stand as its evidence, and is it large enough to anchor a piece and specific enough to act on? Yes: that is the core insight. Write it in one or two sentences, then a sentence saying why it matters. No single survivor contains the rest, but the survivors fall into two or more groups, each with one survivor that contains its own group and passes the other three tests: say the run is carrying that many concepts, split the run, and package each. None pass: say the synthesis is unfinished and return to step 3. Do not nominate the strongest fragment as core.
...
Then order them. Does this angle use a term or a fact that only an earlier angle established? Yes: place it after that angle. No: the two are independent, and the package says so rather than implying a sequence that is not there.

**6. Ground every angle in something a reader could point at.** Per angle: the named example, the number, the moment someone observed, the contrast that makes the point land. Separate what is already in hand from what is missing. Can you reach the missing specific, in the material already handed over, in a file the request names, or by a search this session can run? Yes: find it and mark it sourced. No: ask once, naming which angle goes generic without it. The requester supplies it: mark it sourced. They do not, or they cannot be asked this run: is it a quote or a fact about a person? Yes: do not construct it; carry the gap. No: would the angle then stand only on a construction? Yes: carry it into step 7 as a gap, and do not construct it. No: construct it and mark it constructed. Quotes and facts about people carry their source and register per `standards/conventions.md`. Nothing is invented to fill a hole, and an angle standing only on invented evidence is not standing.
...
Boundaries are scope information, not hedging. A boundary tells a reader where to use the insight; a hedge tells them nothing and costs the claim its force. Do the boundaries name a context where the insight holds and a context where it does not? Yes: write those two or three sentences. They come out as everywhere, or as nowhere: return to step 4. This is that failure surfacing late. Do not ship a hedge in their place.
...
Then the gate, before anything is handed on. Does the package show a vague core insight, an angle that only elaborates the core, no specific on any angle, nothing that survived step 3, or boundaries covering everything? No: it may go on to the gate below. Yes: take the first match. Nothing survived step 3: name what is weak and stop. This is the first fail on that point: go back one pass to the step that owns it, step 4 for the core insight or the boundaries, step 5 for an elaboration, step 6 for specifics. That pass already happened: name what is weak and stop. Do not hand the package on. Drafting cannot repair a concept, only conceal it for a paragraph.
