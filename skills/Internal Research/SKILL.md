...
- **scope**, optional: a root or directory to constrain the scan. Default breadth is the workspace's composed roots, enumerated per the constitution's Workspace Model. A named directory that does not exist is not widened to the composed roots.
...
Enumerate the file tree within scope using the host's directory-listing capability, mapping directories and their contents. List every directory in scope, its own files first and then each nested directory, and do not stop because a name looks unrelated.
...
- The search backend caps its result set. A search that stopped early, or returned exactly its stated maximum, reports that cap and the count; one you cannot tell is complete reports the count and that the set may be capped, and is not called complete. Suggest narrowing scope; never silently truncate.

### Step 4: Read and extract

For each file that matched by path (Step 2) or by keyword (Step 3), read the opening through the frontmatter, the title, and the first qualifying excerpt paragraph, and read every H2 and H3 in the file, including past that opening. The file ends before a qualifying paragraph: that opening is the whole file. Do not read past it to hunt for a better excerpt. Keyword line numbers come from the search in Step 3.

**Read cap: at most 30 files.** More than 30 matched: prioritize by keyword-match count, ties by path ascending, read the top 30, and report "[N] files matched. Showing the 30 with the most keyword hits. Narrow the scope or name a subdirectory for the remaining [N minus 30]." A credential-bearing file is never among the reads.
...
- **Headings:** every H2 and H3 in the file, in document order.
...
- **Excerpt:** the first paragraph of body text that is not a heading, not frontmatter, not a table header, and not a comment, cut at 100 words, finishing a word the cut falls inside. None in that opening: "No excerpt available." The excerpt is the opening paragraph verbatim to that cut, never a selection, a rephrase, or a compression.
...
Order the cards by signal strength: files with keyword matches first, sorted by match count descending, ties by path ascending, then files carried on path relevance alone, by path ascending.
