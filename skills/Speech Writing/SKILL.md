...
Speaker context is the most-skipped and the most load-bearing. Where the speaker can say what the audience should feel or do, use that. Where they cannot, name the gap. The offer, made only in that case, waits until Step 2 has loaded the guide: where that guide states what the occasion is for, offer that, in the guide's own words, as the starting point, and flag it in the delivered draft. Where no guide is loaded, or the guide states no such function, ask, and do not invent one. Do not guess silently. Load the bound `voice` here; if it is unbound, or the constitution's Workspace Model counts it as unavailable, stop and route to `skills/Build Voice/` before any outlining.
...
4. On explicit approval, lock the throughline and the stage order; neither moves during drafting. On a revision direction, or a cut naming a beat or a stage to remove, revise the outline only, never draft prose mid-gate, and re-present. On anything else, or no answer, ask which of the three they mean, and do not draft.
...
Where a requested change moves the locked throughline, moves the locked stage order, or adds or removes a stage, return to Step 3 and re-present the outline; do not patch prose around a broken outline. Where it changes wording inside a locked stage, revise that prose and stay in the draft. Where you cannot tell, return to Step 3, and do not patch.
...
- **Speaker cannot name the outcome.** Where the speaker cannot say what the audience should feel or do, name the gap, offer the loaded guide's stated function in the guide's own words, and flag that working assumption in the delivered draft. Where no guide is loaded, or the guide states no function, ask, and do not invent one.
...
- **Patching prose around a broken outline.** Where the change moves the locked throughline, the locked stage order, or adds or removes a stage, or you cannot tell, return to Step 3. Prose written to bridge a broken outline hides the break rather than fixing it. Where it does not, revise the wording and stay in the draft.
