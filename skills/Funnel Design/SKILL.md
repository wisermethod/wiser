...
1. **Ground the funnel.** Establish four things before designing anything: the business model (what is sold, to whom, at what price), the campaign goal in one sentence (leads, sales, signups, applications), the audience's awareness state (does it know the problem exists, that solutions exist, that this solution exists), and what the requester can realistically produce in pages, emails, and time. Bound `about` supplies the first; `<context>` may supply the rest. Can you name the product, the buyer, the price, the goal in one sentence, the awareness state, and what they can produce? Yes: design from those. Any of them missing: one round of specific questions covering the missing ones, including what is the product, who buys it, and what do they pay. Still unanswerable after that round: which is still missing? The product, the buyer, or the price: the business model is undecided, which is strategy rather than architecture; name `experts/Marketing Strategist/` as where it is settled, and stop. Only the goal, the awareness state, or what they can produce: carry each as a stated assumption and continue; Step 8 asks again for what they can produce. A funnel with no clear offer is a path to nowhere.
...
4. **Specify the sequences.** Every stage that runs on email gets a specified sequence, never a bare label. Which row of the selection table in `sequence-patterns.md` matches the campaign goal? Check the row that names two audiences first: a launch to an existing list that also takes new arrivals matches it and not the generic launch row. It matches: use both sequences it names, then give the cadence, the length, and each message's job in the progression. Otherwise take the first row that matches and use that sequence, the same way. No row matches, including an event reminder, a replay follow-up, or a post-purchase upsell: design the sequence from that file's timing principles and mark it outside the catalog. Would one person be inside two sequences at once? No: leave both as specified. Yes: would sending both exceed the inbox-frequency ceiling that file states? No: leave both running and say so. Yes, and the requester has said to suppress one or to merge them: do that. Yes, and they have not: hold the second until the first completes, and say so. Do not send both past that ceiling. Deadlines, scarcity, and last-chance language appear only where they are true; an urgency the audience learns to disbelieve costs more than the sale it wins.
...
8. **Size it to what can be built.** Hold the finished blueprint against what Step 1 said the requester can produce. Did Step 1 record how many pages, emails, and how much time they can produce? No: ask, and do not call the blueprint done on an unstated capacity. Yes: are the blueprint's pages and emails within those numbers, and can they be built in the time stated? Yes: deliver it. No: say so and offer the minimal version, the smallest set of stages that still runs end to end, with the rest named as later additions. More pages and emails than they can build is not a richer funnel, it is a document.
...
Then the gate: hand the blueprint, with its stated assumptions and the `<context>` it was built from standing in for a strategy where none is on record, to `experts/Marketing Strategist/` in a second context that did not design it. It judges the funnel type against the business model and the audience's awareness state, and the stage progression against the strategy, and returns ship or revise with each finding tied to one of those two things; the findings are worked. What changed after the verdict? The funnel type, or the stage progression, meaning a stage added, removed, reordered, or its purpose or exit action rewritten: re-gate. A correction that leaves the type and the stage progression as the verdict saw them: it clears without another review. You cannot tell whether the progression changed: re-gate. A declined review is named in the delivery.
