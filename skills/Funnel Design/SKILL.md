...
1. **Ground the funnel.** Establish four things before designing anything: the business model (what is sold, to whom, at what price), the campaign goal in one sentence (leads, sales, signups, applications), the audience's awareness state (does it know the problem exists, that solutions exist, that this solution exists), and what the requester can realistically produce in pages, emails, and time. Bound `about` supplies the first; `<context>` may supply the rest. An answer too vague to name the product, the buyer, the price, the goal in one sentence, the awareness state, and what they can produce gets one round of specific questions covering the missing ones. Still unanswerable after that round: the product, the buyer, or the price means the business model is undecided, which is strategy rather than architecture, so name `experts/Marketing Strategist/` as where it is settled, and stop. Only the goal, the awareness state, or what they can produce still missing: carry each as a stated assumption and continue; Step 8 asks again for what they can produce. A funnel with no clear offer is a path to nowhere.
...
4. **Specify the sequences.** Every stage that runs on email gets a specified sequence, never a bare label. Check the row of the selection table in `sequence-patterns.md` that names two audiences first: a launch to an existing list that also takes new arrivals matches it and not the generic launch row, and uses both sequences it names. Otherwise take the first row that matches. No row matches, including an event reminder, a replay follow-up, or a post-purchase upsell: design the sequence from that file's timing principles and mark it outside the catalog. Then give the cadence, the length, and each message's job in the progression. Where one person would be inside two sequences at once and sending both would exceed the inbox-frequency ceiling that file states, suppress one or merge them when the requester has said to, and otherwise hold the second until the first completes, and say so. Under that ceiling, leave both running and say so. Do not send both past that ceiling. Deadlines, scarcity, and last-chance language appear only where they are true; an urgency the audience learns to disbelieve costs more than the sale it wins.
...
8. **Size it to what can be built.** Hold the finished blueprint against what Step 1 said the requester can produce. No record of how many pages, emails, and how much time: ask, and do not call the blueprint done on an unstated capacity. Within those numbers, and buildable in the time stated: deliver it. Otherwise say so and offer the minimal version, the smallest set of stages that still runs end to end, with the rest named as later additions. More pages and emails than they can build is not a richer funnel, it is a document.
...
Then the gate: hand the blueprint, with its stated assumptions and the `<context>` it was built from standing in for a strategy where none is on record, to `experts/Marketing Strategist/` in a second context that did not design it. It judges the funnel type against the business model and the audience's awareness state, and the stage progression against the strategy, and returns ship or revise with each finding tied to one of those two things; the findings are worked. A later change to the funnel type, or to the stage progression (a stage added, removed, reordered, or its purpose or exit action rewritten), re-gates the blueprint; a correction that leaves the type and the stage progression as the verdict saw them clears without another review, and a change you cannot tell re-gates. A declined review is named in the delivery.
