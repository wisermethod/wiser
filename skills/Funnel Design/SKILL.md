...
4. **Specify the sequences.** Every stage that runs on email gets a specified sequence, never a bare label. Which row of the selection table in `sequence-patterns.md` matches the campaign goal? Take the first match. The row that names two audiences: use both sequences it names, then give the cadence, the length, and each message's job in the progression. Any other single row: use that sequence, the same way. No row matches, including an event reminder, a replay follow-up, or a post-purchase upsell: design the sequence from that file's timing principles and mark it outside the catalog. Would one person be inside two sequences at once? No: leave both as specified. Yes: would sending both exceed the inbox-frequency ceiling that file states? No: leave both running and say so. Yes, and the requester has said to suppress one or to merge them: do that. Yes, and they have not: hold the second until the first completes, and say so. Do not send both past that ceiling. Deadlines, scarcity, and last-chance language appear only where they are true; an urgency the audience learns to disbelieve costs more than the sale it wins.
...
8. **Size it to what can be built.** Hold the finished blueprint against what Step 1 said the requester can produce. Did Step 1 record how many pages, emails, and how much time they can produce? No: ask, and do not call the blueprint done on an unstated capacity. Yes: are the blueprint's pages and emails within those numbers? Yes: deliver it. No: say so and offer the minimal version, the smallest set of stages that still runs end to end, with the rest named as later additions. More pages and emails than they can build is not a richer funnel, it is a document.
