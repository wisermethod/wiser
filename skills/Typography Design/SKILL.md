...
Select a ratio from the domain context using the Scale Ratios table. A responsive pair is valid when the project spans two of those domains, one on mobile and one on desktop: the smaller ratio on mobile and the larger on desktop, and state both. Where two domains are named and they are not a mobile surface and a desktop surface, ask which domain governs, and do not average the ratios. Where none of the table's domains is named, use 1.250, the table's recommended default, and say so. That default is the same one the ambiguous-direction pitfall states. State the ratio and its reasoning, and tie the reasoning to context rather than taste.

### 2. Scale generation

Apply the ratio to a 16px (1rem) base as a five-level scale: caption (minus 1), body (0), subheading (plus 1), heading (plus 2), display (plus 3). Add a plus-4 hero step only when the brief asks for a hero step larger than display and the domain is marketing, editorial, or a landing page. Where the domain is one of those and the brief does not ask, keep five levels, and say a plus-4 step can be added if they want a hero larger than display. In any other case, keep five levels, and do not offer the extra step. Generate the scale as a config, a base size, a ratio, and a viewport range, so every size derives from it rather than being set individually.

### 3. Font selection

Map the brand personality to font characteristics using the Font Archetypes table, then verify availability against Google Fonts, system fonts, or a user-provided family. A family they supplied or mandated is the primary, per the brand-mandated pitfall, unless it is a prohibited default. Name the Font Archetypes row whose characteristics it shares most, and say so; that row is the one the availability and partner choices below use. Where they supplied none, use the one row that matches the personality the brief names. Where two rows match, ask which personality governs, and do not blend the rows. Where none matches and they cannot name a personality, use Plus Jakarta Sans or DM Sans, the pitfall's default, and say so. Where they named a personality the table does not contain, ask, and show the six rows. Do not start from the catalog. Where one row matches and they named one of its recommended families, use it when it is on Google Fonts, a system font list, or supplied by them. Where they named none, take the families in the order the row lists them, use the first that is available that way, and tell them the other families in the row. Where the chosen family is not available, compare the other families in the same row on x-height, weight range, and width, use the one that shares more of those three than the others, and name the three. Where two share the same count, ask which. Where they do not pick, do not ship a family. Where none in the row is available, ask before leaving the archetype. Never take a primary family from the prohibited defaults. System fonts are a valid primary only when they asked for them or said performance outweighs personality. Do not switch to them on your own. Keep one family when a mandated family covers both display and body, and add a partner, stating which characteristics make it compatible, when they also asked for one. Where it covers only display or only body, add one partner from the archetype row that contrasts with it, and state the pairing rationale. Where they asked for a second family for contrast, such as a display serif with a body sans, add that one family and state the pairing rationale. Otherwise keep one family across several weights. A third family follows the more-than-two pitfall. Stop at two unless that pitfall's insist outcome applies.
...
Set a line height for each scale level per the Line Heights table. Where the text is not light on a dark background, or you cannot tell the background, use the table's value unchanged, and where you cannot tell, say the light-on-dark adjustment was not applied. Where it is light on dark and they asked for more room, add 0.1, the top of the table's range; otherwise add 0.05. Take the vertical rhythm base from the body line-height (16px at 1.5 is a 24px base) and space sections in multiples of it (24, 48, 72, 96). Set letter spacing to 0 for body. Use a heading value they named from -0.02em to -0.01em, or a caption or all-caps value from +0.05em to +0.08em. Where they named none, use -0.01em on headings and +0.05em on captions and all-caps labels, the end of each range closer to zero, and say the rest of the range is open. Where they named a value outside the range, ask before using it.

### 6. Fluid sizing

Use clamp() for every size above body, derived from the scale config in the Utopia-style approach: a mobile base of 16px, a desktop base, a mobile ratio, a desktop ratio, and a viewport range of 320 to 1280px, from which each step's single clamp() value follows. The mobile ratio and the desktop ratio are the ratio, or the responsive pair, that step 1 already chose. Use a desktop base they named inside the reference range of 18 to 20px. Where they named one outside it, ask before using it. Where they named none, use 18px, the bottom of that range, and say a base up to 20px is open if they want a larger desktop body. Include the rem offset so text never collapses on small screens. Do not fluid-size button text, labels, or UI elements, which need viewport consistency, nor caption size, where fluid scaling at small sizes hurts readability.
