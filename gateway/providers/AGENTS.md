...
| `proxy({ providerAccountId, toolkit, endpoint, method, body, parameters, binary_body })` | `{ status, data }` or `{ status, error: { code, endpoint, method } }` | Response headers are dropped. Never the vendor's body on failure; the same-domain rule is enforced here if the provider has one. **`parameters` is an array of `{ name, value, type }` where `type` is `header` or `query`, and it is not the SDK-documented `in` key, which the REST API rejects outright.** Settled 2026-09-17 by a four-arm probe against the real API, including a deliberately wrong digest to prove the header value reached the vendor, because two vendor documentation pages disagreed. Confirm a header shape against the API rather than against a repository fixture, which can encode a shape the API refuses |
