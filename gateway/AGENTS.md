...
| The audit log, one JSON line per `execute`, `start_connect`, `connect_status` and `disconnect`, including the ones that stopped: time, harness, role, op, action, service, module, privilege, resolution path, provider account id, status, duration, correlation id, and the two version facts `model` and `calibrated_model_version`. No input, no output, no header. A line that cannot name a version fact carries null rather than omitting the key | `~/.wiser/gateway/audit.jsonl`, same modes | Every call |
| The classifier presence file: `attached` (a classifier loaded and `readClassifierKey` is not null), `classifier_dirs` (the absolute directories that loaded), `pid`, and `started_at`. Never a key and never a value derived from a key | `<home>/classifier-status/<harness>.json`, mode 0600 in a 0700 directory, `<home>` the screened gateway home. The file name is the `--harness` label when that label is a single safe path segment | On a normal start, after classifiers load. Not on `help`, `--check`, `--call`, or `--route`. A write that fails does not stop the gateway |
...
**What it serves.** The seven action ids `wiser.route.roster`, `wiser.route.ask`, `wiser.gate.check`, `wiser.decide.choice`, `wiser.decide.batch`, `wiser.recall.rank`, `wiser.browser.pick`. `search_actions` and `describe_action` list them when a classifier is loaded, and do not mention them when none is. A host that reads tools meets them there; a host that reads instructions meets `wiser.route.ask` in the constitution. When `search_actions` is given an absolute `owning_root`, a query, and a loaded classifier, it puts one `wiser.decide.choice` through the first-party execute path and may list the accepted action first.
...
**The two version facts.** Every first-party execute writes `model` and `calibrated_model_version` on the audit line, including a roster call, an uncalibrated refusal, and a call that returns before the adapter (`needs_subscription`, `invalid_arguments`, `needs_confirmation`, `needs_connector` / `undeclared`, `denied`). The values are taken from `result.meta` before any conversion of the result, so a `vendor_error` mapping cannot drop a model the adapter supplied. A member is kept only when it is a non-empty string of at most 128 characters, and is null otherwise: absent `meta`, a non-string, an empty string, and a longer string are null. Omitting either key is the failure. `meta` is `{ model, bar, calibrated_model_version }`, present on every adapter result, and it is not part of any action's answer. `model` is the last envelope the call read: a non-empty string, the string `unknown` when an envelope was read and its `model` is not one, or null when no envelope was read. The string `unknown` is kept as that non-empty string; the gateway does not invent it from a non-string. `bar` is the dotted path of the bar a probability was compared against (`routing.door_r2`, `routing.target_r2`, `gate.T_fail`), or null when no comparison happened. A value that is not one of those three paths is null. `calibrated_model_version` is that bar's version, including null where the bar carries null, and null when `bar` is null. The two selections come apart: an alias compares a bar without posting, and a malformed envelope can be read without a comparison. A version mismatch stays the adapter's status object. The line records `unavailable` for it and does not record `reason`, `calibrated_on` or `answered_by`. On a mismatch, `calibrated_model_version` is the checked bar's version.

**The answer.** Each first-party action declares its success shape beside its input in `FIRST_PARTY_ACTIONS`, and the gateway checks the adapter's result against it. A result that fails answers `{ status: 'unavailable', reason: 'malformed answer' }`. The reason is fixed. Nothing is rounded, defaulted or coerced, and the adapter's message is not copied. The check does not apply to a gateway status, to `unavailable`, `below_threshold` or `roster_unknown`, to any other adapter status string, to a `vendor_error` result, or to the reserved `meta` key. A `below_threshold` that names `p` must name a probability; a missing `p` is left as the adapter returned it. `wiser.route.ask` allows `confidence: null`. `wiser.browser.pick`'s `verb` is required and not type-checked. `wiser.decide.batch` checks each entry of `answers`, and a malformed answer is that same refusal.
...
**Rendering** is what makes it safe, and it applies to every value whatever its schema says. A schema cannot be trusted for this: of the 90 declared fields across the 19 shipped `confirmation: always` actions, 40 are bounded by their own declaration against control characters, and four of the seven connectors carrying one declare no pattern at all. **Re-measured 2026-09-20**, twice, after the confirmation policy moved 12 actions off `always` and then adversarial review moved 7 on: the net is 24 before, 13 at the first pass, 19 now. The counted rule is a field bounded by an `enum`, by a non-string scalar type, or by a `pattern`, over the top-level properties of each action's published `input`. **The bounded proportion rose from 18 percent to 44 percent and the conclusion is unchanged**, because more than half of every declared field is still unbounded and four connectors of seven declare no pattern at all. A rule gated on the schema would print nothing for those. So every value is escaped against the Unicode classes `Cc`, `Cf`, `Cs`, `Zl`, `Zp` and `Default_Ignorable_Code_Point`, plus the backslash and the double quote; capped, with a cut marked by the true length and a fingerprint; and fitted into a summary whose every component is budgeted so the 2000-character limit holds unconditionally.
...
**None of it reaches the audit line.** `AUDIT_FIELDS` in `src/audit.js` is a closed set with no input field. The two version facts, `model` and `calibrated_model_version`, are members of that set, and a test asserts a rendered value does not appear in `audit.jsonl`.
