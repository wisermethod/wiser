...
**The two version facts.** Every first-party execute writes `model` and `calibrated_model_version` on the audit line, including a roster call, an uncalibrated refusal, and a call that returns before the adapter (`needs_subscription`, `invalid_arguments`, `needs_confirmation`, `needs_connector` / `undeclared`, `denied`). The values are copied from `result.meta` when that object is present and are null when it is not. Omitting either key is the failure. `meta` is `{ model, bar, calibrated_model_version }`, present on every adapter result, and it is not part of any action's answer. `model` is the last envelope the call read: a non-empty string, the string `unknown` when an envelope was read and its `model` is not one, or null when no envelope was read. `bar` is the dotted path of the bar a probability was compared against (`routing.door_r2`, `routing.target_r2`, `gate.T_fail`), or null when no comparison happened. `calibrated_model_version` is that bar's version, including null where the bar carries null, and null when `bar` is null. The two selections come apart: an alias compares a bar without posting, and a malformed envelope can be read without a comparison. A version mismatch stays the adapter's status object. The line records `unavailable` for it and does not record `reason`, `calibrated_on` or `answered_by`. On a mismatch, `calibrated_model_version` is the checked bar's version.
