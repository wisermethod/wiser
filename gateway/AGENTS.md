...
**What it serves.** The six action ids `wiser.route.roster`, `wiser.route.ask`, `wiser.gate.check`, `wiser.decide.choice`, `wiser.decide.batch`, `wiser.recall.rank`. `search_actions` and `describe_action` list them when a classifier is loaded, and do not mention them when none is. A host that reads tools meets them there; a host that reads instructions meets `wiser.route.ask` in the constitution. When `search_actions` is given an absolute `owning_root`, a query, and a loaded classifier, it puts one `wiser.decide.choice` through the first-party execute path and may list the accepted action first.
