...
| The auth provider project key | The platform user-config path `SETUP.md` names (`auth-provider.env`). The gateway reads it and never writes it | When `--env` is passed, or when that default file exists |
...
**Three things the gateway never writes**, stated because their absence is the design: a token, a key or a credential of any kind, anywhere; the input or output of an action, anywhere; a file inside the plugin.
