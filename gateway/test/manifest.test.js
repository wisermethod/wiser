...
test('--check validates all shipped connectors and prints one JSON object', async () => {
  ...
  assert.deepEqual(ids, ['bing', 'clarity', 'cloudflare', 'courtlistener', 'dataforseo', 'figma', 'github', 'google', 'google-ai', 'google-vision', 'hubspot', 'huggingface', 'linkedin', 'microsoft', 'monday', 'notion', 'pagespeed', 'replicate', 'stripe', 'supabase', 'tinyfish', 'usebouncer', 'vercel', 'zoho', 'zoom']);
  ...
  assert.ok(obj.actions.includes('google-ai.translate.text'));
});
