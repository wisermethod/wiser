...
description: Lists and reads Outlook messages through a separate Outlook grant
version: 0.1.0
---

# Microsoft Outlook

Lists and reads Outlook messages through a separate Outlook grant.
...
This Microsoft facade has only `outlook`. No action accepts `user_id`.
...
Send, reply, draft, delete, move, calendar, contacts, To Do, Teams chats, OneDrive, and SharePoint.
...
- `needs_connect`: use Connect Account for `microsoft` / `outlook` in its own human turn.
