...
### What `path` requires, and why the schema cannot say it

`upload_file` refuses a `path` that does not name a readable file on the machine running the gateway. **No published schema can carry that**, because it is filesystem state rather than a shape: the same string is valid on one machine and not on another, and valid at one moment and not the next. The manifest declares `path` as a required string and stops there, and this sentence is the rest of the rule.

It is the one action in the family whose row in `gateway/test/agreement-baseline.json` is permanent. Every other row there is a schema to correct; this one records a requirement a schema cannot express, and the same is true of `create`'s `dir` and of a path-string `files` array.
