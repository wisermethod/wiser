...
- **Claude Cowork.** Add `wisermethod/wiser` as a marketplace and install `wiser`. **This host is the one that has not been run yet**, so read the rest of this bullet as what the plugin asks of a host, not as what has been observed on this one: the working folder you attach is meant to be the owning root, the writing is meant to land there and not in this tree, and the constitution is meant to load from `AGENTS.md`. None of those three has been measured in Cowork, and neither has the tool half, where a tool installs its packages into its own directory on first call and the gateway is a process you attach. Until someone has run it, treat every one of them as an honest stop rather than a promise.
