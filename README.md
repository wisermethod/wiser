...
- **Claude Cowork.** Add `wisermethod/wiser` as a marketplace, install `wiser`, then attach the working folder the work is about. All three of the things that matter were run here and they hold: the skills arrive with the install, the constitution loads from `AGENTS.md`, and work lands in the attached folder rather than in this tree. **The tool half is the honest stop.** Cowork runs Claude in an isolated virtual machine, so a tool that installs its own packages on first call is untested there, and the gateway is a process on your own machine that the VM has no route to. Treat a tool step and a connector step as unproved on Cowork until someone has run one.
