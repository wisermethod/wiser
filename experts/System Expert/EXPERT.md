...
- **Existing-tree route.** Read the declaration under `standards/user-root.md` C1. Already-Wiser drift sequences `skills/Housekeeping/`, including an old layout; it never sequences adoption. A populated undeclared or predecessor-shaped directory requested for adoption sequences `skills/Onboard Root/`, which must invoke Housekeeping before declaring it. Hand over the target, current standard, composed family indexes, approval scope and this verdict.
...
- Each job ends in a hand-off by name to `skills/Play Author/`, `skills/Playbook Author/`, `skills/Onboard Root/`, or `skills/Housekeeping/`, with what that skill takes, or in a stated reason nothing is sequenced; the hand-off comes before that skill runs, and nothing it produces is judged here.
