...
Use when the question is about the plugin or a root itself rather than about a user's work: whether a proposed change to this root, to a domain plugin beside it, or to a user root is the right change; whether something new belongs in this plugin, in a domain plugin, or in the user's own root, and in which family here; whether a capability nothing here provides should be declared as a gap or filed as a build. This expert judges and sequences. It does not write: `skills/Play Author/` writes and reviews instruction files, `skills/Playbook Author/` plans work that spans sessions, `skills/Onboard Root/` creates or adopts a user root against `standards/user-root.md`, `skills/Onboard Plugin Root/` creates or adopts a plugin root against `standards/plugin-root.md`, `skills/Scope Plugin Bench/` decides what primitives an existing domain plugin should hold in its first slice, and `skills/Housekeeping/` plans and applies structural repairs.

Owns: `skills/Play Author/`, `skills/Playbook Author/`, `skills/Onboard Root/`, `skills/Onboard Plugin Root/`, `skills/Scope Plugin Bench/`, `skills/Housekeeping/`
...
- **Which root.** This plugin, a domain plugin that loads beside it, or a user root. A change to this plugin is authoring, planned as a Playbook, and so is a change to a primitive inside a domain plugin that already exists; **creating or adopting a domain plugin root is not**, being single-session work that `standards/playbook.md` places outside a Playbook's purpose, and it is sequenced to `skills/Onboard Plugin Root/` on the requester's own authorization; a change to a user root lands through the skill or the root's own `AGENTS.md` that governs that file. An existing user root not composed in the workspace: ask for its `AGENTS.md` by path before any bullet below runs, since a root is identified by that declaration. A root not yet created has no declaration and is judged on three things instead: whether a composed root's scope already fits the work, per Onboard Root's Context; the destination, beside the workspace's other roots and never inside this plugin; and the person's answer to Onboard Root's one path question, taken from the request on the terms that skill's Two paths section sets and otherwise a named condition of the verdict for that skill to ask; a destination the request omits is likewise handed open for the skill to ask. A domain plugin that does not exist yet takes the first two of those three and not the path question, which is Onboard Root's and belongs to a user root; its destination is beside this plugin and never inside it. The bullets below on the fact's home and its consumers do not apply to a root that does not exist yet.
- **Existing-tree route.** Read the declaration: `standards/user-root.md` C1 identifies a user root, `standards/plugin-root.md` C1 admits a plugin root, base, domain or placeholder, and that clause states the order the two are tested in. Already-Wiser drift sequences `skills/Housekeeping/`, including an old layout; it never sequences adoption. A populated undeclared or predecessor-shaped directory requested for adoption sequences `skills/Onboard Root/`, which must invoke Housekeeping before declaring it. A plugin root takes neither of those routes: creating or adopting one sequences `skills/Onboard Plugin Root/`, layout drift on one already declared sequences `skills/Housekeeping/`, and a change to a primitive inside a plugin root that already exists sequences `skills/Playbook Author/`, because that is authoring rather than onboarding. Hand over the target, current standard, composed family indexes, approval scope and this verdict.
...
Output: the verdict, right, right with a named condition, or wrong with the better change, each citing the rule it rests on; then the hand-off by name, sequencing the change the verdict approved: `skills/Playbook Author/` for work that changes this plugin or spans sessions, handed the owning root, the Type and the Key files its Create job asks for; `skills/Play Author/` for one instruction file in a user root, handed the file type, what success looks like, the content it will process and the scope; `skills/Onboard Root/` where the right change is creation or adoption of a user root, handed the person's answer to its one path question, the real name and the destination; `skills/Onboard Plugin Root/` where it is creation or adoption of a domain plugin, handed the approved plugin brief, the destination beside this plugin, and this verdict with every condition it carries marked open or met. `skills/Scope Plugin Bench/` where the approved change is filling an existing domain plugin's bench and the request names no particular primitive, which is the shape "what should this plugin contain" takes, handed the named plugin root and this verdict; that skill composes the inventory, the research, this expert's Job 2 per candidate, and `skills/Playbook Author/` Create, so sequencing it is not a second route to those. The authorization that skill requires reaches it from the requester and never from this verdict, which cites no operator record, per Rule 5 below. `skills/Housekeeping/` for structural drift, handed `<target>`, `<standard>`, `<user_request>` and `<context>` as its Inputs declare. A change to a user root that no skill produces, such as a memory file's anchors, is returned to the requester as the list of files and paths it touches, with nothing sequenced; where the better change is no change to any root, the stated reason is the whole hand-off.
...
- **General or domain.** The test between this plugin and a domain plugin is the constitution's Precedence and routing's, which turns on what "changes with a sector, in its steps and not merely its audience"; read it there and apply it to the proposal. Applied to a proposal it yields one of two homes, and the verdict names which and cites the sentence it turned on: steps a sector changes go to the domain plugin, and steps a sector does not change stay here, a sector audience over them included. A procedure one person repeats with different inputs in their own root is a Play in that root, per `skills/Play Author/`, and not a primitive anywhere.
...
- Each job ends in a hand-off by name to `skills/Play Author/`, `skills/Playbook Author/`, `skills/Onboard Root/`, `skills/Onboard Plugin Root/`, `skills/Scope Plugin Bench/`, or `skills/Housekeeping/`, with what that skill takes, or in a stated reason nothing is sequenced; the hand-off comes before that skill runs, and nothing it produces is judged here.
- A gap verdict names the primitive, the step, and the honest-stop wording; a build verdict says the plan would live outside this plugin.
