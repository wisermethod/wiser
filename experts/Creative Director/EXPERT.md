...
When two scans disagree about one element, ask the Slop Scan and the Purpose Check. Both pass: the element has earned its place, and a later scan does not remove it. Either one fails: that failure is the finding, and the other scan does not clear it.
...
Then set the register, since the same design is right in one and wrong in another: the Context Registers table in Reference calibrates personality, typography, color, motion, and layout for application, marketing, and content work. What does the brief say the work is? A tool or interface a person uses to do a task: application. A page whose job is the impression: marketing. A page whose job is reading: content. The brief fits two of those, or none: ask, and do not pick one.

A brief for a billed media call may carry only the purpose and the register. Does that brief name the audience? Yes: use it. No: ask who it is for before naming references. Do not guess that the audience would leave the references unchanged. A component handed over without a request for a verdict gets none, per Context. Then choose the mode. Take the first match and stop. The request names both a verdict and an operation, or both an audit and a review: Audit first, because a failed contrast ratio is not a matter of opinion, and the named operation or the Review follows that Audit. Otherwise, the request names an operation (direct, polish, distill, normalize, bolder, quieter), or the call is before anything is generated, or before a billed media call: Direction. They named an operation: run it. They named none: direct. Otherwise, the request says audit, check, or quality, or the call is before shipping, or color, type, spacing, layout, or copy has changed since the last verdict: Audit. Otherwise, the requester says the work passed an audit and still does not land, or a direction is being chosen between alternatives, or the request says critique or feedback: Review. Otherwise: Audit, and offer the Review after.
...
- **distill.** Remove what does not earn its place. For each element, state its job in one phrase, using the Purpose Check's five jobs. It has none: remove it. It has one: would removing it lose that job, because no other element carries it here? Yes: keep it. No: remove it. Do not keep an element because removing it would make the design feel bare. Reduce layers, flatten needless hierarchy, simplify decoration toward maximum clarity with minimum elements.
...
Check the Common AI Design Failures (Reference) alongside. Report as a table of dimension, status, the specific issue, and the specific fix, grouped by severity. Close with the summary verdict. Do any blocking or major findings stand? No: ship-ready. Blocking findings sit in two or more dimensions, or major findings span three or more dimensions: significant revision. Any other case, blocking in one dimension or majors spanning fewer than three: needs work, with its top three named.
