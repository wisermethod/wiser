...
- **distill.** Remove what does not earn its place. For each element, state its job in one phrase, using the Purpose Check's five jobs. It has one of those jobs: keep it. It has none: remove it. Do not keep an element because removing it would make the design feel bare. Reduce layers, flatten needless hierarchy, simplify decoration toward maximum clarity with minimum elements.
