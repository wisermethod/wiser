...
2. Classify each operation against the supplied evidence:
   ...
   - Catalog absent, vendor requires a minted short-lived token: **not** a custom toolkit and **not** local-file. A custom toolkit may declare only `NO_AUTH`, `API_KEY` or `DCR_OAUTH`, so it cannot carry OAuth unless the vendor publishes a dynamic client registration endpoint. Plan a bring-your-own-OAuth auth config on a toolkit the provider already ships for that vendor, carrying the OAuth client the person registers and the narrowest scopes the operations need, with proxy execution at absolute URLs. The provider is expected to hold the refresh token and mint access tokens; no credential enters this plugin. Class A. **Read the vendor's generated schema for the scopes each operation accepts, never its prose pages**; the scope requested is the grant's blast radius. **Hang the config off a toolkit carrying no other config, and check before creating**, because the adapter refuses to start a connection on a toolkit carrying more than one. Two cases this branch does **not** cover, and each ends in a blocker rather than in local-file: a vendor the provider ships **no** toolkit for at all, and a vendor whose OAuth needs dynamic client registration, which the shipped adapter builds no row for. Name which one in the blocker.
   - Custom toolkit injection cannot match the vendor, **and no bring-your-own-OAuth route exists for it**: local-file is last resort, still class A. **Never for a credential that expires**: a file cannot hold a token with an hour's life, so a minted-token vendor that reaches this bullet is a blocker, not a local-file plan. Require evidence of the mismatch, such as a sanitized 401 or 9106-class failure after checking the toolkit and header contract. An expired key or insufficient scope alone does not prove injection is impossible. Plan `--secret <service>=<abs file>` or an explicit Provides `secrets:<platform>` binding, never `memory/secrets/`.
...
3. Split modules by resource domain and grant privilege. What privilege does `<api_evidence>` state for the grant? It states read, write, or admin, including a phrase that names only one of those: record that word, independently of action risk. It states none of the three: block that privilege and name the evidence needed. Do not infer admin from a write action, and do not infer read from an action that looks safe. List every action id, endpoint or catalog mapping, execution preference, risk, confirmation, and excluded operations. **For input, the plan states the action's published `input` schema and the module's own checks as one decision, not input validation as one item**: the gateway applies the published schema before the module runs and no connector carries a validator, so what the plan leaves unsplit is what the two halves must agree on. `standards/script-contract.md` Published input schema binds it and `gateway/test/agreement.test.js` proves it. A plan that says a bound in prose and leaves the schema to the author produces the defect that audit found 189 times in fourteen connectors, none of them written carelessly: the two halves were simply written at different moments. What confirmation does the action take? It deletes, overwrites, sends, or publishes: `always`. It is spend, and any cost-driving input is unbounded by its own declaration, or the price is not published at all: `always`. It is spend, and every cost-driving input carries a declared upper bound (`maximum`, `maxItems`, `maxLength`, or an `enum`), so one call's worst case is a small stated amount: `none`, and `CONNECTOR.md` states that worst case, not only the base rate. Every cost-driving input is bounded, but the worst case of one call is not a small amount: `always`. Both limbs apply: `always` wins, and a bounded worst case does not buy an exemption for an action that also deletes, overwrites, sends, or publishes. It is a read, and none of those limbs apply: `none`. It is another write, and the request itself says a single approval is the confirmation this write should carry: `once`. It is another write, and the request does not say that: `always`. Do not raise a read above `none` because it feels sensitive, and do not use `none` for a write.
...
5. Return the plan with a verdict, evidence and unresolved assumptions, destination, file changes, module/action table, fake test cases, and a separate human-connect check. Can you name one supported auth route, and does every action have the confirmation step 3 set? Yes, step 2 named no blocker, and step 3 blocked no privilege: verdict approved, with every unresolved assumption listed beside it. An approved verdict opens Connector Author's gate. A step 2 blocker, a privilege step 3 blocked, or an action whose confirmation is still blank: verdict blocker. A blocker names the evidence or decision needed before it can write. Do not approve a plan that still leaves the credential route to be guessed.
