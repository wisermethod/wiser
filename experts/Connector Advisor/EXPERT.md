...
5. Return the plan with a verdict, evidence and unresolved assumptions, destination, file changes, module/action table, fake test cases, and a separate human-connect check. Can you name one supported auth route, and does every action have the confirmation step 3 set? Yes, and step 2 named no blocker: verdict approved, with every unresolved assumption listed beside it. An approved verdict opens Connector Author's gate. A step 2 blocker, or an action whose confirmation is still blank: verdict blocker. A blocker names the evidence or decision needed before it can write. Do not approve a plan that still leaves the credential route to be guessed.
