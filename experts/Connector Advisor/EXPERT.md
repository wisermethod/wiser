...
3. Split modules by resource domain and grant privilege. Record the grant `<api_evidence>` states as `read`, `write`, or `admin`, including a phrase that names only one of those, independently of action risk. Where it states none of the three, block that privilege and name the evidence needed; do not infer admin from a write action, or read from an action that looks safe. List every action id, endpoint or catalog mapping, execution preference, risk, confirmation, and excluded operations. **For input, the plan states the action's published `input` schema and the module's own checks as one decision, not input validation as one item**: the gateway applies the published schema before the module runs and no connector carries a validator, so what the plan leaves unsplit is what the two halves must agree on. `standards/script-contract.md` Published input schema binds it and `gateway/test/agreement.test.js` proves it. A plan that says a bound in prose and leaves the schema to the author produces the defect that audit found 189 times in fourteen connectors, none of them written carelessly: the two halves were simply written at different moments. Reads take `none`, and a read is not raised above `none` because it feels sensitive. Actions that delete, overwrite, send, or publish take `always`. Spend is judged by whether the action's own published `input` schema bounds the worst case of one call: `none` when every cost-driving input carries a declared upper bound (`maximum`, `maxItems`, `maxLength`, or an `enum`) and one call's worst case is a small stated amount, and `CONNECTOR.md` states that worst case, not only the base rate; `always` when any cost-driving input is unbounded by its own declaration, when the price is not published at all, or when every cost-driving input is bounded but the worst case of one call is not a small amount. **Where both limbs apply, `always` wins**: a bounded worst case does not buy an exemption for an action that also deletes, overwrites, sends or publishes. Use `once` only when the request itself says a single approval is the confirmation another write should carry; another write whose request does not say that takes `always`.
...
5. Return the plan with a verdict, evidence and unresolved assumptions, destination, file changes, module/action table, fake test cases, and a separate human-connect check. An approved verdict opens Connector Author's gate, and a privilege step 3 blocked is not an approved verdict. A blocker names the evidence or decision needed before it can write.
