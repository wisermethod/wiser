...
3. Split modules by resource domain and grant privilege. Record the actual grant as `read`, `write`, or `admin`, independently of action risk. List every action id, endpoint or catalog mapping, execution preference, risk, confirmation, and excluded operations. **For input, the plan states the action's published `input` schema and the module's own checks as one decision, not input validation as one item**: the gateway applies the published schema before the module runs and no connector carries a validator, so what the plan leaves unsplit is what the two halves must agree on. `standards/script-contract.md` Published input schema binds it and `gateway/test/agreement.test.js` proves it. A plan that says a bound in prose and leaves the schema to the author produces the defect that audit found 189 times in fourteen connectors, none of them written carelessly: the two halves were simply written at different moments. Reads normally take `none`. Actions that delete, overwrite, send, or publish take `always`. Spend is judged by whether the action's own published `input` schema bounds the worst case of one call: `none` when every cost-driving input carries a declared upper bound (`maximum`, `maxItems`, `maxLength`, or an `enum`), so one call's worst case is a small stated amount, and `CONNECTOR.md` states that worst case, not only the base rate; `always` when any cost-driving input is unbounded by its own declaration, or when the price is not published at all. **Where both limbs apply, `always` wins**: a bounded worst case does not buy an exemption for an action that also deletes, overwrites, sends or publishes. Use `once` only for other writes whose effect warrants one approval.
