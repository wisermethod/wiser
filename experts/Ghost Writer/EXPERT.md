...
- **Uniform sentence rhythm.** Sentences of one length in sequence, so the cadence carries no emphasis and the reader hears the pattern rather than the point. A transition that announces itself is the same fault, rhythm doing a connective's work. **At the head of a sentence, ten are always this fault**, read the way Sentence fragments below is read, without judging whether the sentence carries them: *Additionally*, *Furthermore*, *Moreover*, *In conclusion*, *Consequently*, *Nevertheless*, *Nonetheless*, *Subsequently*, *Conversely*, *In terms of*. **The position is the test**: the same words mid-sentence, or quoted as an example, are not the fault, which is what keeps the list from firing on prose that discusses it. A word carrying a live second sense at the head, *however*, *therefore*, *meanwhile*, *that said*, is judged rather than matched and is deliberately not on the list. Whether a matched transition becomes a finding is the cost test's, as it is for every tell here, and the fix is deletion: a lighter connective in its place reproduces the intrusion.
...
- **Em dashes and en dashes.** A piece leaving this workspace carries neither. Use a comma, a semicolon, or a period in place of one, and "to" for a range; the hyphen is the only dash. `standards/conventions.md` Formatting scopes the rule to what this gate reads and defers the statement of it to here, so this is the only place it is stated in full. **This one is a house requirement rather than a reader judgment, and it is the single exception to the cost test in Commitment 2 and to the drop rule closing this section.** Every other tell here is dropped when the intended reader would not stumble; this one is not, and a reviewer does not have to construct a cost for it before raising it. **That exception is deliberate and it is stated rather than inferred**: the operator set this rule for every piece leaving the workspace, so the question Commitment 1 asks, what this reader would notice, is not the question here and a reviewer who answers it has answered the wrong one. It is also why the decline in Pitfalls does not waive it. Rule 2 is the only other place a finding survives the cost test, and it survives on a fixed cost; this one survives on the requirement itself. **No judgment is involved and the position is not the test**: every em dash and every en dash in the piece is the fault. **Find them by counting the characters, not by reading for them and not by pattern-matching them**, because a search for these two returns a believable zero and a reader skims past them; count `U+2014` and `U+2013` and print a control proving the count ran. The fix is chosen per clause, because a blind substitution to a comma produces wrong prose. **Four things are not the fault**: a quotation, a title, or a source's own words, all reproduced as written; the habit where the bound `voice` names it, per Precedence above; and a feeder file **while it is only a feeder**, because this gate reads one for what it will do to the prose it feeds rather than as a piece that ships. **A feeder that is itself the deliverable is not exempt**, which the Context names for a theme set shipping in its own right, and the same holds for any concept package or voice file handed over as the work product.
