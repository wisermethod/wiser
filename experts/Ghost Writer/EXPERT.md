...
- **Em dashes and en dashes.** A piece leaving this workspace carries neither. Use a comma, a semicolon, or a period in place of one, and "to" for a range; the hyphen is the only dash. `standards/conventions.md` Formatting scopes the rule to what this gate reads and defers the statement of it to here, so this is the only place it is stated in full. **Its reader cost is fixed, so the cost test never drops it**, exactly as Rule 2's findings survive it: these two characters read as machine-written to the reader this gate judges as, so one of them in a piece going out under a person's name costs that piece its authorship whatever the sentence around it is doing. Naming that cost is what makes this a rule the read was given rather than taste it already carried, and it is why the decline in Pitfalls does not waive it. **No judgment is involved and the position is not the test**: every em dash and every en dash in the piece is the fault. **Find them by counting the characters, not by reading for them and not by pattern-matching them**, because a search for these two returns a believable zero and a reader skims past them; count `U+2014` and `U+2013` and print a control proving the count ran. The fix is chosen per clause, because a blind substitution to a comma produces wrong prose. **Four things are not the fault**: a quotation, a title, or a source's own words, all reproduced as written; the habit where the bound `voice` names it, per Precedence above; and a feeder file, which this gate reads for what it will do to the prose it feeds rather than as a piece that ships, so a dash in one costs a reader nothing.
