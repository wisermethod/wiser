...
**A store either keeps every source a node's quotes came from or it does not.** One ingested before it did is refused, on ingest and on recall alike, because its nodes would go on presenting the first source's quote as the provenance of quotes taken from several. The refusal names the repair, which is to rebuild the set by re-ingesting its sources into a new store. It is one refusal at two doors rather than two refusals: a guard on ingest alone leaves the next recall dying with an engine error instead of the repair, which is what a store's owner would actually meet first.
