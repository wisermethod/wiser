...
**A ranked call may ask for the passages alone, and only by asking.** `--candidates-only` returns `part: passage` items and no others, unchanged in every field, ranks still running 1 to N in order, and the result carries `candidates_only` so it says on its face what it did. **The default path is untouched and returns all three parts**, so the paragraph above remains the contract for every call that does not name the flag. It exists because a caller that chooses among candidates is told to ignore two of the three parts, and on a 1,012 passage store those two were **100 of 125 items and 45% of the payload**, measured 2026-09-15. **It is a payload cut and not a time cut**: the traversal it skips was measured at **at most 0.069 seconds of a 15.28 second call**. The flag is refused where it would trim nothing: with `MATCH`, with `--select`, on a databased recipe, and against a store holding no passages.
