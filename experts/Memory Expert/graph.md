...
Graph ingest accepts the extraction shape in `tools/knowledge-memory/references/schemas.md` section 2 and stores `Idea`, `Entity`, and every declared relation type: nodes carry `name`, `definition` or `summary`, `quote`, `source_path`, `quotes`, `source_paths`, and `status`; the relation carries its endpoints, `quote`, `source_path`, and `status`. **A node whose name a later source repeats keeps every source's located quote**, in `quotes` and `source_paths`, paired by position and in ingest order; `quote` and `source_path` keep the first source's. The node is not re-created, so the skip is still counted, and what the skip no longer does is lose the later source's evidence. Relations run from Idea to Idea or Entity. Facts, decisions and open questions are counted as skipped with a named unsupported-table reason.
...
**A store either keeps every source a node's quotes came from or it does not.** One ingested before it did is refused on ingest, because its nodes would go on presenting the first source's quote as the provenance of quotes taken from several, and refused on the passage retrieval paths, whose attachment reads those columns. **The two paths that do not read them are not refused and are not harmed**: a Cypher MATCH returns what the node holds, and a store with no passages at all never reaches the attachment, so both keep working exactly as they did. The refusal names the repair, which is to rebuild the set by re-ingesting its sources into a new store. **Guarding ingest alone would not have been a refusal**: it leaves the next passage recall dying with an engine error instead of the repair, which is what a store's owner would meet first.

Ingest always creates Candidates regardless of extraction status. An existing node name is skipped and counted, and the repeated source's located quote and path are appended to that node rather than discarded; an identical edge is skipped with counts. One extraction is transactional. No graph promote or replay command is provided. Every node and relation needs a located quote and source path. No quote, no node. Knowledge Set Onboarding confirms Pro canon with the human; ingest never sets Canonical automatically.
...
An item from a store holding passages also carries `part`, one of `passage`, `idea`, `related`, or `match` on the Cypher path, and a `related` item carries the `relation`, `direction` and `via` it was reached by, because a typed hop that cannot say which type it followed is not a typed hop. **An `idea` item carries `via` too**, the passage it was reached through, and its `quote` and `source_path` are the pair belonging to that passage's own source, falling back to the first where that source contributed none. An item reached through one source that presented another's quote as its own is the thing that pairing exists to stop. The fallback path over a store with no passages returns the flat shape above and no `part`, which is what makes it a fallback rather than a second contract. An `idea` or `related` item has no `score` or `rank`, which are properties of an embedding row. A passage returned by a selection also carries `candidate_rank`, the position it held in the list the caller chose from, named for what it carries rather than for any one ranking. **A selection is named to the tool, never written to the tree**: `--select` takes the JSON list itself on the command line as well as a path to a file holding it, and a caller that writes the file instead leaves behind a record of which passage answers which question, in a directory later work searches. The list carries each chosen passage's own `score`, `rank`, `vector_rank` and `lexical_rank`, because the selection path does no embedding and cannot re-derive them, and a reordering that hides what it reordered is not auditable. The three parts ride inside `items` rather than beside it, so the one skill that composes graph results sees all of them and the shape above keeps its single home. Items are grouped by rank, passage then its ideas then its relations, because ordering part by part spends the whole evidence budget on passages and no idea ever reaches the composer.

**A ranked call may ask for the passages alone, and only by asking.** `--candidates-only` returns `part: passage` items and no others, unchanged in every field, ranks still running 1 to N in order, and the result carries `candidates_only` so it says on its face what it did. **The default path is untouched and returns all three parts**, so the paragraph above remains the contract for every call that does not name the flag. It exists because a caller that chooses among candidates is told to ignore two of the three parts, and on **one measured call** against a 1,012 passage store those two were **100 of 125 items and 45% of the payload**, measured 2026-09-15; across six questions on that same store the cut ran **110 to 144 of 135 to 169 items**, 1.91x to 2.28x of payload. **It is a payload cut and not a time cut**: the traversal it skips was measured at **at most 0.069 seconds of a 15.28 second call**. The flag is refused where it would trim nothing: with `MATCH`, with `--select`, on a databased recipe, and against a store holding no passages.

**The composer is not deprived by it**, because the composer's call is the selection, not the ranked call: `--select` attaches the ideas of the passages actually chosen, scoped to them, which is the same `attach` the ranked path uses.
