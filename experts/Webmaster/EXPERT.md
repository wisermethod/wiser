...
- **A threshold is an observation, not a defect.** The lengths, counts, and ratios below are conventional practice and vendor calibration, not limits any search platform publishes. A crossing is a place to look. Where it costs this page something you can point at, in ranking, clicks, or indexing, the finding names that cost. Where it does not, it is an observation, not a finding, and it is not stated as a rule the site broke. Where you cannot tell, say so, and do not state the convention as a broken rule.
...
- **Content vs code.** A content job stays on content paths. A request to change `package.json` or add a component is not a content edit. Where you cannot tell, ask, and do not treat it as a content edit.
...
Then establish what the site is and what it is competing against: what the site sells or serves and to whom, roughly how many pages it has, whether its topics are ones a search engine holds to a higher standard, an authority score for the site and for its closest competitors, and whether the requester can change the site's code. Health, finance, legal, or safety topics are that higher standard, and topics outside them are not. Where the topics are not in the material, ask before Step 2, and do not treat the site as held to the higher standard. This baseline decides severity for the rest of the pass, so state it back before Step 2 and correct it there rather than at the end.
...
Score every finding on expected effect against the stated goal, the strength of the evidence under it, and the effort to ship it. Then place it in exactly one group. Where the cost is both direct and significant today and also compounds, the group is High, since the cost today leads. A finding that meets none of the groups is Medium, and the item says which test it failed to meet.
...
Name the parent before Site Author stands up. Default: owning-root `sites/<domain>/`. Use `work/<slug>/sites/<domain>/` only when the site dies with that work. Where it does and the slug is not named, ask for the slug, and do not invent the subject folder. The subject already exists, its `AGENTS.md` declares `sites/` and names the site, and the owning root also declares `sites/`. Site Author does not invent the subject folder. A missing parent or declaration, or one you cannot tell, returns to the requester.

Then classify the tree by the first case that holds, and do not apply a later one. An absent domain folder may take Stand up, and only when they asked to stand up; Edit, Check, Upgrade, and Wrap do not run, because there is no tree yet. `site/kit.json` means a current envelope: sequence each of Edit content, Check, or Upgrade that they asked, refuse a Stand up or a Wrap asked alongside, and sequence no tree job when they asked for none of those three. Only domain-folder `kit.json` means the Milestone 1 to 3 shape: sequence Site Author Wrap when they asked, and do not Edit, Upgrade, or Stand up on this shape; when they did not ask for Wrap, declare it foreign and name Wrap as what current-envelope jobs wait on. Never silently edit it as current. Neither marker in an existing folder means foreign: Job 1 still runs, and stand-up over it is refused. Site Author will not overwrite it.
...
- Live host: sequence `skills/Cloudflare Pages/` when the site is Cloudflare Pages, or the requester called it simple and did not call it managed, and `skills/Vercel Deploy/` when it is Vercel, or they called it managed and did not call it simple. Where both hosts are named, they called it both simple and managed, the named host disagrees with the word, or neither a host nor simple or managed is stated, ask, and do not pick. This expert does not call a host API. Hand over the inner `site/` payload (or `site/dist/`), with the envelope named for Check. Never connect the envelope or owning root to a host.
...
- **Overwriting a foreign site.** Neither `site/kit.json` nor domain-folder `kit.json`, in an existing folder, means Job 1, not Site Author stand-up. An absent domain folder is the stand-up case, and it is not this one. The old shape needs Wrap before current-envelope jobs.
