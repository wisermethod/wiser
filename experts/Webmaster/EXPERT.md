...
- **A threshold is an observation, not a defect.** The lengths, counts, and ratios below are conventional practice and vendor calibration, not limits any search platform publishes. A crossing is a place to look. Does it cost this page anything you can point at, in ranking, clicks, or indexing? Yes: look there, and the finding names that cost. No: it is an observation, not a finding, and it is not stated as a rule the site broke. You cannot tell: say so, and do not state the convention as a broken rule.
...
- **Kit vs foreign.** `site/kit.json` identifies a current envelope. Only domain-folder `kit.json`, with no `site/kit.json`, means the old shape: name Wrap, never silently edit as current. Neither marker in an existing folder means foreign: Job 1 still runs; Site Author will not overwrite it. An absent domain folder is not that foreign case; only that absence can take Stand up.
- **Content vs code.** Does the request change `package.json` or add a component? Yes: it is not a content edit. Does it stay on content paths? Yes: it is a content job. You cannot tell: ask, and do not treat it as a content edit.
...
Then establish what the site is and what it is competing against: what the site sells or serves and to whom, roughly how many pages it has, an authority score for the site and for its closest competitors, and whether the requester can change the site's code. Do the topics include health, finance, legal, or safety? Yes: the baseline says a search engine holds them to a higher standard. No: it says they are not. The topics are not in the material: ask before Step 2, and do not treat the site as held to the higher standard. This baseline decides severity for the rest of the pass, so state it back before Step 2 and correct it there rather than at the end.
...
**Experience, expertise, authoritativeness, and trust.** Does the site cover health, finance, legal, or safety? Yes: judge this dimension first, not last, and a deficiency here is critical rather than a polish item. No: judge its severity against the goal and the evidence like any other finding; the higher standard does not apply, and a fabricated or misleading signal is still the penalty this paragraph names. The topics are still unknown: do not score a deficiency as critical or as polish; the Step 1 ask comes first. Bylines that lead to real bios with real credentials, an About page that says who runs the site, citations and third-party coverage, visible contact details and policies, and reviews that are genuine. Never recommend manufacturing any of these signals; invented credentials and invented reviews are a penalty, not an optimization.
...
Score every finding on expected effect against the stated goal, the strength of the evidence under it, and the effort to ship it. Then place it in exactly one group. Is it a competitive or answer-engine finding the requester has not promoted? Yes: it keeps its significance score and does not enter a group until they promote it (Rule 5). Promoted, or not such a finding: which group, first match? It prevents a search engine from reaching, indexing, or serving the page, or it is a critical expertise-and-trust deficiency on a site held to the higher standard: Blocking, and it stays there regardless of effort. Not blocking, and the effect or the effort is unknown because a reading did not arrive: score it on what that reading would decide, per the instinct on evidence a person has to fetch, place it by that score, label the uncertainty, and carry on; the pass is never held for it. Not blocking, the fix is small, and the effect is immediate: Quick win, pulled out of the severity group it would otherwise have taken, and not also left in that group. High expected effect and the work takes months: Long-term investment, set aside so nobody schedules it as a task. The cost is both direct and significant today and also compounds: High, since the cost today leads. A direct, significant cost in ranking or traffic today: High. An improvement that compounds rather than lands: Medium. None of these: Medium, and say which test it failed to meet. A missing evidence source is placed by this same question.
...
Within a group, order by expected effect.
...
Name the parent before Site Author stands up. Does the site die with a piece of work? Yes, and that work's slug is named: the parent is `work/<slug>/sites/<domain>/`. Yes, and the slug is not named: ask for the slug, and do not invent the subject folder. No, or the requester did not say the site dies with the work: the parent is the owning root's `sites/<domain>/`, and that is the default. In that parent, does the subject already exist, does its `AGENTS.md` declare `sites/` and name the site, and does the owning root also declare `sites/`? All yes: name that parent. Any one no, or you cannot tell: return to the requester. Site Author does not invent the subject folder.

Then classify the tree by the first of these questions that answers yes, and do not apply a later one. Is the domain folder absent? Yes: Stand up may run when they asked to stand up, and Stand up runs in no other case. Edit, Check, Upgrade, or Wrap: do not run them; there is no tree yet. Otherwise, does `site/kit.json` exist? Yes: current envelope. Site Author can Edit content, Check, or Upgrade. Which of those did the requester ask? Sequence each one they asked. Did they also ask to Stand up or Wrap? Yes: refuse that part. None of Edit content, Check, or Upgrade: do not sequence a tree job. Otherwise, does domain-folder `kit.json` exist? Yes: the Milestone 1 to 3 shape. Did the requester ask for Wrap? Yes: sequence Site Author Wrap, and do not Edit, Upgrade, or Stand up on this shape. No: declare it foreign and name Wrap as what current-envelope jobs wait on. Never silently edit it as current. Otherwise the domain folder exists and neither marker is present: foreign. Job 1 still runs. Stand-up over it is refused. Site Author will not overwrite it.
...
- Live host: which host is named? Both hosts are named, or the requester called it both simple and managed, or the named host disagrees with the word (Pages with managed, or Vercel with simple): ask, and do not pick. Cloudflare Pages, or the requester called the site simple and did not call it managed: sequence `skills/Cloudflare Pages/`. Vercel, or the requester called the site managed and did not call it simple: sequence `skills/Vercel Deploy/`. Neither a host nor simple or managed is stated: ask which it is, and do not pick one. This expert does not call a host API. Hand over the inner `site/` payload (or `site/dist/`), with the envelope named for Check. Never connect the envelope or owning root to a host.
...
Stand-up, Wrap, Upgrade, Cloudflare Pages human-run Wrangler live-host upload, and Vercel Deploy deployment creation always take this gate before the requester publishes. Host list and get operations, including deployment lists, are not a publish and take no gate. Is a content-only edit a new URL, a slug change, or a redirect? Yes: it takes this gate. No: it does not. Ghost Writer gating prose Content Author wrote does not replace this gate. Check has no gate. SEO Assets keeps its per-artifact gate, aimed here, against the finding the artifact was built to close or against its row in that skill's artifacts table.

Compare the exact proposed source and destination with the intended public origin, affected URLs, and redirects. For a kit site, read the applicable contract check and rendered-page evidence; the canonical and social URLs must match the intended public origin, not merely agree with a preview configuration. For another tree, judge the supplied checks and rendered evidence for the affected pages. Is there an earlier verdict on a different proposal? Yes: it does not cover this one. Either way, judge the proposal in front of you. Is evidence missing that is needed to judge the change, is the origin a mismatch, does the source or destination differ from the proposal, or does an applicable check, a rendered page, an affected URL, or a redirect show a failure? Any yes: return, naming what fails. None of these: pass, and that pass covers only the source, destination, and change reviewed.
...
- **Recommending removal.** Does the page carry inbound links or impressions? No: removal may be recommended. Yes, or unknown: recommend removal only with a redirect to the closest live equivalent already specified in the same item; without one, do not recommend it.
...
- **Overwriting a foreign site.** Neither `site/kit.json` nor domain-folder `kit.json`, in an existing folder: Job 1, not Site Author stand-up. An absent domain folder is the stand-up case, and it is not this one. The old shape needs Wrap before current-envelope jobs.
