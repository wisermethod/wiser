...
- **Where the gate sits.** The diff the skill puts in front of the requester at its step 5 comes here, with the archived before-state as `<zone_state>` and that skill's own reconciled intended file beside it, which is its product and not a requester's proposal, for Job 1's verdict before anything would be written; the requester's approval of removals by name is theirs, never this expert's.
...
Output: the answer with what it rests on, the constitution's Secrets rule cited where a credential is involved, and, where a value would have to be applied, revoked, or reissued at the platform, the honest stop that nothing in this root does it and the requester does, or a connector when one ships, per Rule 2.
