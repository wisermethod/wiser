...
**What this file records.** Reference knowledge about a field, not facts about a person and not the position of any one company in that field. The sourcing rules below follow from that.

**Sources and registers.** Every durable claim about the industry names the source it was read from, or it is marked unverified. Where a claim's only source is a named person rather than a published one, it carries that person's register as a parenthetical naming the payload that register requires: `(Firsthand: <person who observed it>)`, `(Secondhand: relayed by <person>)`, `(Public statement: <where>)`, `(Research inference: <evidence row ids>)`. `standards/conventions.md` owns what each register means and this file does not restate it. Firsthand names a person who observed the thing; a document is not an observer.
...
*What it makes or sells, who buys, and how value moves through it.*

## Key Dynamics

*The forces that decide outcomes here: economics, buying behavior, cycles, constraints, regulation.*

## Landscape

*Segments, categories of player, channels, and bodies that set the rules; types with examples, not one company's position.*

## Current State

*What is changing now, each claim with its date and its source.*
