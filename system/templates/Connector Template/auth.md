...
**If you mention a blueprint here, carry this sentence verbatim**, from
`connectors/shared-text.md`, which says why it is worded exactly this way and what it cost to
learn. Say more after it if this connector needs it; do not state the rule differently.
...
**This section is a projection of `connectors/shared-text.md`, and the only thing you edit in it
is the slot.** Every shipped guide on the `catalog` route carries this text byte for byte;
`gateway/test/guide-conformance.test.js` fails when one diverges. Replace `{VENDOR ROUTE}` with
one to three sentences saying what a person does at this vendor to revoke or rotate the
credential, ending in a period, and change nothing else. A rule that applies to every connector
...
**If this connector uses the `local-file` provider, delete everything above and say so instead.**
That adapter holds no grant, always refuses `revoke`, and never reports `ABSENT`, so `disconnect`
cannot take its credential down and no record of it is ever removed. `connectors/usebouncer/auth.md`
is the worked example, and the gate exempts a connector serving no `catalog` module by reading its
manifest. Promising a gateway teardown a connector cannot perform is the defect this template
exists to prevent.
